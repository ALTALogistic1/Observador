...
- **N'alerte jamais sur un appel d'offres OUVERT — ce n'est pas le produit.** *Frontière posée le 14 septembre 2026, en instruisant six agrégateurs d'appels d'offres.* **Un avis d'appel d'offres ouvert n'a pas de gagnant** : il dit que l'organisme qui le publie a un besoin, et il s'adresse à quiconque veut soumissionner. **C'est une occasion de soumissionner, pas un prospect** — le métier de MERX, de J360 et des agrégateurs commerciaux, et il est déjà servi. **FALKYE détecte une entreprise qui vient de faire quelque chose, et en déduit ce dont elle aura besoin.** *Un avis d'ATTRIBUTION, lui, est exactement notre matière : il nomme une entreprise qui vient de gagner.* ⚠️ **La seule exception concevable est un profil dont le client EST l'État** — et elle bute sur le fait que le donneur d'ouvrage n'existe pas comme entité *(registre, D43)*, donc elle ne s'ouvre pas aujourd'hui. *Écrire cette frontière évite de reproposer chaque année un flux d'appels d'offres comme s'il manquait au portefeuille.*
...
**La règle ci-dessous ne vise que la troisième nature, et c'est ce qui l'empêche de dériver.** Le produit **ne porte jamais de jugement négatif sur ce qu'il a lui-même décidé de présenter** — non par délicatesse, mais **par cohérence** : il l'a choisi. **Mais il le gradue, et la gradation reste parfaitement lisible.** Ce qui est interdit n'est pas de distinguer, c'est de **dénigrer le bas de l'échelle** : *le plus court n'est pas mauvais, il est simplement moins.* **Exemple travaillé, et le seul du corpus : les trois libellés de la Pertinence** — *Repéré*, *Aligné*, *Sur mesure* *(spéc. 8.1)*. Chacun dit quelque chose de positif, l'ordre ne fait aucun doute, et **la gradation s'explique sans qu'aucun niveau ait à s'excuser.** Et **une échelle emprunte le jugement de la convention à laquelle elle ressemble** — une échelle en `A/B/C` porterait le bulletin scolaire que le moteur le veuille ou non. *C'est une illustration, pas un précédent : aucune échelle du produit n'a jamais porté ces lettres (journal, cas 16).*

**À quel document du dépôt cette section s'appliquerait en entier — aucun aujourd'hui.** Le corpus décrit des instruments internes et des données lues; il n'est presque jamais concerné, et **aucun document commercial n'est versé au dépôt.** La règle est écrite d'avance parce qu'elle se tranche mal sous pression : **le seul document du dépôt auquel la section 16 s'appliquerait en entier est celui qui parle à quelqu'un qui va payer. Le corpus est interne; celui-là serait la parole du produit.** Un tel document irait **au dépôt** — *un document hors du dépôt n'est repéré par personne quand il vieillit, et ce serait la seule affirmation du projet vendue à quelqu'un* — **et hors de `docs/spec/`**, *parce qu'une finalité présentée ne se range pas parmi les instruments internes : ce serait la confusion de cette section même, un étage plus haut.* Avec deux règles déjà éprouvées sur le README : **ne jamais affirmer un fait que le corpus porte, y renvoyer**, et **tout chiffre porte sa date devant, ou il n'apparaît pas**.

*⚠️ **Cette clause n'a pas d'objet et n'en annonce aucun.** Elle ne dit pas qu'un document commercial est attendu, ni qu'un document montré au passage en deviendra un. **Un document montré n'est pas un document versé** *(journal, cas 31)*.*
...
***Aucun cas d'origine, et c'est assumé.*** *Cette règle a été posée avant tout incident. L'exemple qui lui servait d'appui — une échelle `A/B/C` devenue `A/AA/AAA` — **n'en était pas un** : il avait été donné en conversation pour l'expliquer, puis consigné au journal comme un fait, où il a tenu lieu de précédent à quatre écrits **(journal, cas 16)**. **La règle tient sans lui.** Elle est **démontrée, non fondée**, par les trois libellés de la Pertinence — Repéré, Aligné, Sur mesure *(spéc. 8.1)*. La section 16 porte par ailleurs un incident réel, **le cas 19**, mais il concerne la forme de livraison, pas le vocabulaire. **C'est la règle 4 appliquée à la charte elle-même : un principe qu'aucun incident n'a forcé se lit comme tel, plutôt que d'emprunter un précédent.***
