...
**À quel document du dépôt cette section s'applique en entier.** Le corpus décrit des instruments internes et des données lues; il n'est presque jamais concerné. **La présentation commerciale est le seul document du dépôt auquel la section 16 s'applique en entier. Le corpus est interne; celui-là est la parole du produit à quelqu'un qui va payer.** Elle vit à **`docs/presentation-commerciale.md`** — *au dépôt, parce qu'un document hors du dépôt n'est repéré par personne quand il vieillit, et que c'est la seule affirmation du projet qui soit vendue à quelqu'un; **hors de `docs/spec/`**, parce qu'une finalité présentée ne se range pas parmi les instruments internes — ce serait la confusion de cette section même, un étage plus haut.* Deux règles avec : **elle n'affirme jamais un fait que le corpus porte, elle y renvoie** *(même règle que le README)*, et **tout chiffre porte sa date devant, ou il n'apparaît pas**.
