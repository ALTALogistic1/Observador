...
6. **Sous-comptes et territoires assignés, avec rôles — reclassé comme répartition de volume, pas comme sécurité.** Le vrai besoin identifié chez les personas Radar+ réels (développement économique régional, cabinets multi-agents) est de distribuer automatiquement les bonnes notifications à la bonne personne selon son secteur/territoire assigné, pour réduire le bruit — pas d'empêcher un collègue de la même organisation de voir les données d'un autre territoire. **Construit et testé, avec la mise en garde de Claude Code toujours valide et documentée : ce n'est pas une frontière de sécurité tant qu'il n'y a pas d'authentification réelle.** Mais puisque le besoin réel est la répartition de volume et non l'étanchéité entre utilisateurs d'une même organisation, **l'urgence d'une authentification réelle avant le premier client payant est révisée à la baisse** — ce reste un vrai prérequis à construire, mais pas un bloqueur immédiat au lancement commercial de Radar+, tant que le produit ne prétend jamais être une frontière de sécurité dans son propre matériel de vente (voir charte, section 6).

Notées pour une feuille de route plus lointaine : rapports exportables automatisés en marque blanche, authentification SSO/SAML complète, authentification réelle par utilisateur (prérequis à construire avant de présenter les rôles comme une séparation stricte, mais plus classée comme urgente au sens du lancement commercial).
