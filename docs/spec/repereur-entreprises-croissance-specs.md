...
Vue tableau de bord listant les dossiers cumulatifs sous forme de pastilles/cartes, réservée aux plans Radar et Radar+ (absente d'Écho, dès 29,99$/mois — Écho n'est pas un palier gratuit, seules les sources qu'il utilise le sont). Chaque carte affiche :
...
3. **Tableaux de bord agrégés par territoire** : au-delà des prospects un à un, une vue agrégée ("X entreprises en croissance détectées dans votre région ce trimestre, réparties par secteur") — sert un besoin concret de reddition de comptes pour des personas comme le développement économique régional, qui doivent justifier leur propre impact à un conseil ou à un palier gouvernemental. **Construit, mais limite réelle découverte en testant, non résolue :** le secteur d'activité d'une des sources internes est un texte libre si granulaire (~211 valeurs distinctes sur 311 notifications réelles) que l'agrégation par secteur reste peu utile telle quelle — deux entreprises du même secteur peuvent avoir un libellé différent, ce qui casse le regroupement. **Décision ouverte, pas encore tranchée :** investir dans un vrai regroupement (SCIAN/NAICS) pour rendre l'agrégation utile, ou lancer tel quel avec une agrégation plus grossière en attendant.
   **Correction additionnelle requise :** les entreprises détectées uniquement via une source hors Québec n'ont jamais de secteur classifiable par ce mécanisme et tombent systématiquement en "non classé", peu importe leur vrai domaine — solution proposée : réutiliser `clients_cibles.yaml` (déjà indépendant de toute source) comme dimension d'agrégation complémentaire, plutôt qu'un deuxième registre à construire. **Aucun libellé affiché à l'utilisateur ne doit jamais nommer une source précise** (charte, section 6, élargie) — la dimension doit être présentée par territoire ou par catégorie neutre, jamais "secteur REQ" ou équivalent, y compris dans les libellés d'exportation ou les messages système.
...
8. **Assistance à la configuration du profil par IA, à deux niveaux — seul usage de ML retenu dans tout le produit.** Contrairement au moteur de score (confiance/pertinence), qui reste en règles simples et documentées pour rester explicable, ceci touche uniquement l'accueil du profil : l'utilisateur décrit en texte libre ce qu'il fait/offre, l'IA propose les sphères de besoin et mots-clés correspondants. **Le résultat est toujours une proposition que l'utilisateur confirme ou corrige — jamais une classification silencieuse qui affecte ses résultats sans qu'il la voie.**
...
### 8bis. Lien sphère↔service plusieurs-à-plusieurs, pondéré, et dimension "qui" (client cible) — livré et validé (392/392 tests, migration réelle appliquée)
...
**Deux exigences additionnelles, non négociables — design final confirmé :**

**Simplicité d'utilisation, un seul point d'entrée conversationnel :** nouvelle commande (`profile configurer-besoin` ou équivalent) prenant deux textes libres — description du service, description du client cible — et produisant UNE proposition complète, jamais une série d'étapes séparées. Le Niveau 1 tourne sur les deux textes (réutilisé tel quel, pas un nouveau mécanisme), et son résultat (compte de mots-clés matchés par candidat) devient directement les poids proposés, normalisés sur l'échelle 0-100 déjà en usage. Une seule confirmation applique tout le bloc d'un coup (sphères pondérées ET client cible, y compris `aucune_restriction`). Les poids existent dans `ProfileNeedSphere.poids`, mais ne sont jamais l'interface — seulement une donnée de transparence secondaire, jamais quelque chose que l'utilisateur doit lire ou manipuler pour configurer son profil. `profile lier-sphere` (raffinement manuel) reste disponible, jamais une étape obligatoire.

**Départage en cas de tie exact — résolu par généralisation du schéma, pas par une règle mécanique.** Un tie exact au Niveau 1 (au moins deux candidats au score maximal) devient un deuxième déclencheur d'escalade au Niveau 2, à portée réduite : pas "quelle sphère dans tout le catalogue" (le mode déjà construit pour une correspondance nulle), mais "laquelle de ces N candidates déjà à égalité est la plus pertinente, et avec quel poids relatif" — un nouveau mode dans `assistance_sphere_ia.py`. Le schéma de sortie contraint retourne un **ensemble** de `{sphere_id, poids}` plutôt qu'un seul `sphere_id` — chaque `sphere_id` toujours tiré du catalogue fermé (garde-fou structurel inchangé), mais plusieurs à la fois. C'est le même changement de schéma dont le mécanisme sphère↔service général a de toute façon besoin — un seul schéma généralisé, pas deux formats séparés. **Aucune règle mécanique de départage n'est nécessaire nulle part** : le poids EST déjà le classement (dérivé, jamais une colonne séparée), donc "principal" tombe naturellement du côté le plus élevé. Inversion utilisateur sans jamais toucher un pourcentage : commande qui nomme simplement la sphère voulue comme principale (`profile definir-sphere-principale`), le système ajuste les poids en arrière-plan — généralise au-delà d'un tie à deux, fonctionne pareil avec trois sphères liées ou plus.

**Validation de bout en bout complétée**, contre le moteur réel (`engine._traiter_entreprise_pour_profil`/`deliver_notification`) : trois cas testés (désaccord confiant, accord, absence de signal). Preuve directe que le désaccord ne produit aucun malus — son score (60/100) est identique au cas "absence de signal" (60/100), pas juste affirmé. Zéro tentative de livraison créée pour le cas hors-profil, contre une vraie tentative pour les deux autres cas — confirme que le routage fonctionne réellement, pas seulement en calcul interne. Scénario documenté dans `docs/ARCHITECTURE.md`/`docs/STATUT_RESEAU.md`, même format que le scénario d'authentification.
...
- **RDPRM (registre des droits personnels et réels mobiliers, Québec)** — gratuit à consulter à l'unité, payant (11$/nom, 4$/NIV). Capte les inscriptions de garanties prises par des institutions financières sur des biens d'entreprise (équipement, inventaire, véhicules) lors d'un financement. Signal précoce, indirect mais public, de financement d'expansion. **Deux mécanismes distincts, à ne jamais confondre :**
  - **Mécanisme 1 — déjà documenté, opérateur.** Alexandre effectue lui-même une recherche RDPRM ponctuelle et l'importe dans le logiciel (section 9) — FALKYE achète cette donnée pour son propre usage produit, exactement comme n'importe quelle autre source payante acquise pour le service (ex. TheirStack). Le résultat entre dans le pipeline standard, visible par tout profil dont la sphère correspond à l'entreprise concernée. **Placement par palier, conditionnel à la clarification légale déjà notée (personne physique vs entité juridique dans les garanties RDPRM) :** si l'achat et l'usage commercial de la donnée sont confirmés légaux sans restriction, RDPRM mécanisme 1 va dans **Écho** — cohérent avec le principe de la charte (section 13) qu'une source nécessaire à la qualité de base du produit n'est pas automatiquement reléguée à un palier supérieur simplement parce qu'elle est payante. Si un doute légal persiste, RDPRM mécanisme 1 va dans **Radar** plutôt qu'Écho, pour contenir l'exposition à un volume d'utilisateurs plus restreint en attendant une clarification complète.
  - **Mécanisme 2 — nouveau, portail client (Radar minimum).** Un client Radar paie lui-même, via un portail dédié, sa propre recherche RDPRM pour une entreprise de son choix. **Le résultat reste strictement scopé à l'instance de CE client — jamais partagé, réutilisé, ou redistribué pour enrichir les résultats d'un autre utilisateur.** Contrairement au mécanisme 1, ceci n'entre jamais dans le pipeline standard partagé. **Vérification légale déjà faite, prudence confirmée :** aucune clause explicite trouvée dans les conditions d'utilisation publiques du RDPRM sur la réutilisation d'un résultat de recherche — mais puisque ce résultat reste ici strictement dans l'instance du client qui l'a payé (jamais redistribué à un tiers), le risque est jugé minimal, contrairement à un scénario de mise en commun qui, lui, resterait à proscrire. **Blocage avant construction :** personne n'a encore examiné le format réel d'un document RDPRM (quelles colonnes, quel format Excel) — nécessaire pour construire la configuration `champs_pertinents`-style par sphère (quel champ RDPRM compte pour quelle sphère), prête à s'appliquer automatiquement dès qu'un client achète son document. Un premier achat réel servira à la fois de test et de référence pour cette configuration.
  - **Lien direct vers la page de recherche (mécanisme 1) :** `https://www.rdprm.gouv.qc.ca/Consultation/` (consultation assistée par nom — la recherche doit utiliser le nom légal exact enregistré au Registre des entreprises du Québec).
...
- **Corporations Canada (ISED)** — **nouvelle découverte, équivalent fédéral du REQ, mais pancanadien** : couvre toutes les entreprises incorporées sous une loi fédérale (partout au Canada, anglais et français), publié en **téléchargement gratuit en vrac, mis à jour quotidiennement**, avec en plus une **vraie API en temps réel** (plus complet que le REQ à cet égard). Répond directement au besoin de couverture en anglais Canada.
  ...
  - **Limite à noter, et vraie valeur ajoutée précisée :** ne couvre que les entreprises incorporées fédéralement. **Au Québec, cette source n'ajoute rien** — toute entreprise opérant au Québec doit déjà s'immatriculer au REQ, peu importe son type d'incorporation, donc une entreprise fédérale y apparaît déjà. **La vraie valeur est dans les 9 autres provinces**, où aucune source Piste équivalente au REQ n'existe (accès inégal — gratuit en Ontario/Nouvelle-Écosse pour la recherche de base, payant en Colombie-Britannique, fermé en Alberta) : Corporations Canada y sert de **Piste partiel pan-canadien** (confirmé fonctionnel, tier moyen seulement — changement d'adresse — jamais fort, voir section recherche de sources Piste) pour le sous-ensemble d'entreprises incorporées fédéralement, notre seul point d'ancrage, même partiel, hors Québec à ce jour.
...
**Correction critique livrée et validée (417/417 tests) :** pour toute entreprise sans NEQ (hors Québec ou non résolue au REQ), la correspondance par nom était exacte uniquement, jamais floue — contrairement à la résolution REQ elle-même. Un vrai bogue de doublons existait déjà dans la base réelle (76 paires à similarité ≥90% jamais reconnues comme la même entreprise). Correction : correspondance floue (`rapidfuzz.fuzz.WRatio`) ajoutée, restreinte aux entreprises sans NEQ uniquement (jamais contre une entreprise déjà résolue au REQ), avec deux seuils — fusion automatique au-dessus de 95 (bonifiée si la ville concorde), journalisation dans `DiagnosticJournal` comme candidat à confirmer manuellement entre 90 et 95, jamais de fusion automatique silencieuse dans cette zone. **Garde-fou additionnel trouvé en validant contre la vraie base, avant de considérer le travail fini :** une fusion erronée a d'abord eu lieu entre deux compagnies à numéro légalement distinctes ("9519-3801 Québec inc." / "9519-3850 Québec inc.", le matricule numérique diffère complètement, seul le suffixe générique "Québec inc." gonflait le score) — restaurée depuis la sauvegarde, puis **toute comparaison floue entre deux compagnies à numéro est désormais interdite structurellement**, ces noms étant génériques par nature et jamais fiables à comparer par similarité, peu importe le seuil. **Action requise de ma part :** 20 candidats de fusion restent en attente d'examen manuel, consultables via `diagnostic lister --type candidat_fusion_entreprise` — certains sont probablement des faux positifs (ex. "Groupe TVA inc." / "Groupe A. inc.").

### Recherche de sources Réflexion par sphère de besoin (3 septembre 2026) — candidats vérifiés

**Cadre légal général, confirmé au Québec, à appliquer systématiquement :** le régime par défaut d'un organisme gouvernemental réserve tous les droits, y compris sur une donnée publique — "indexer dans une base de données" y est explicitement interdit sans autorisation. Exception : une diffusion via un vrai portail de données ouvertes (Données Québec, licence gouvernementale ouverte du Canada). **Toujours vérifier la variante exacte de licence sur la fiche du jeu précis** (CC-BY autorise le commercial, CC-BY-NC l'interdit — ex. Hydro-Québec est en CC-BY-NC) — jamais présumer qu'un organisme applique la même politique partout. Voir charte, section 12.

**1. RACJ — Registre des titulaires de permis de détaillant d'alcool en vigueur (Québec) — priorité 1, aucun obstacle.** Bars, restaurants, centres de vinification/brassage, épiceries, cidreries. Bulk (CSV/JSON/XLSX + API Données Québec), mise à jour hebdomadaire, **CC-BY 4.0 confirmé, usage commercial permis, aucune autorisation requise**. **Livre le NEQ directement** (deuxième source seulement, avec l'OQLF, à le faire) — aucun appariement approximatif nécessaire. Champ `Capacite` (nombre de personnes) = proxy de taille d'établissement, rare dans les sources gratuites. Sphères : restauration/gestion alimentaire (cible), commerce de détail, agroalimentaire, construction/rénovation, immobilier/aménagement, entretien ménager, SST, sécurité physique, assurance/gestion des risques. Tier fort pour une nouvelle inscription, moyen pour un changement de titulaire. **Calibration requise** (aucune date de délivrance dans le registre, diff hebdomadaire nécessaire) : nouveau `NoPermis`/`Neq` = ouverture; `Neq` inchangé mais `Titulaire` différent = changement de propriétaire; **`Capacite` en hausse sur un établissement existant = agrandissement**, signal probablement le plus intéressant et le plus facile à rater.

**2. CIPO / IP Horizons (fédéral) — priorité 2, licence commerciale explicite.** Marques de commerce, dessins industriels, brevets. Bulk (XML ST.96 hebdomadaire, jeux CSV/TXT trimestriels), **SFTP d'automatisation officiellement documenté**, gratuit. Licence CIPO explicite pour usage commercial (mondiale, libre de redevance, révocable, droit de sous-licence, attribution obligatoire, régie par le droit de l'Ontario). Type Réflexion (aucun identifiant d'entreprise — NEQ ni numéro fédéral —, rapprochement par nom/adresse comme les licences Vancouver/Toronto). Sphères, la meilleure trouvaille cross-sphère de cette recherche : franchisage/développement de réseaux (une marque de commerce est l'actif central d'un réseau de franchise), ingénierie/design industriel (dessin industriel = nouveau produit physique), automatisation/robotique (brevets dans les classes pertinentes), juridique/conformité réglementaire, et en second rang analytique d'affaires (couverture faible), marketing/vente, commerce de détail, agroalimentaire, import/export. Tier fort (dépôt de marque, dessin industriel) à moyen (brevet seul, délai de publication 18+ mois). **Calibration requise :** exclure les désignations Madrid de titulaires étrangers (dominent le volume, aucune croissance locale), exclure les dépôts au nom de cabinets d'agents, filtrer sur l'adresse du requérant dans le territoire ciblé, distinguer un premier dépôt d'un renouvellement/cession.

**3. Registre fédéral des lobbyistes (Commissariat au lobbying) — priorité 3, licence ouverte sans ambiguïté.** Bulk (ZIP de CSV + dictionnaire de données), licence gouvernementale ouverte du Canada, usage commercial permis explicitement. Type Réflexion (nom de société seulement). Sphères : relations publiques/communication corporative (cible), juridique/conformité réglementaire, planification stratégique, financement/accès au capital (champ indicateur de financement public reçu). Bonus structurel : chaque enregistrement déclare des **sujets** (environnement, fiscalité, industrie, transport, etc.) qui se mappent directement sur les sphères. Tier moyen — un **nouvel** enregistrement (pas une mise à jour) indique une entreprise qui commence à défendre ses intérêts au fédéral, souvent lié à une expansion. **⚠️ Point de conformité obligatoire, cohérent avec la charte section 6 :** ce jeu contient des noms de personnes physiques (lobbyistes, titulaires de charge publique) — à exclure à l'ingestion comme une exclusion universelle, au même titre que le bruit administratif du REQ, jamais seulement filtré au calcul de pertinence. **Calibration requise :** ne garder que les nouveaux déclarants et nouveaux sujets ajoutés, exclure les grandes sociétés/associations enregistrées en permanence dont les mises à jour périodiques ne signalent rien.

**4. Établissements alimentaires — Ville de Montréal — priorité 4, complément du RACJ.** Bulk (CSV/GeoJSON/SHP), mis à jour aux quelques jours, CC-BY 4.0 confirmé, commercial explicitement autorisé. ⚠️ Une clause à surveiller sans être bloquante : la Ville "proscrit tout usage malveillant [...] notamment toute tentative d'identifier une personne, une entreprise" — vraisemblablement visée à la ré-identification depuis des données anonymisées, mais une ambiguïté réelle vu que "identifier une entreprise" décrit littéralement la fonction de FALKYE, à garder en tête. Champ `Statut` = `En traitement` (ouverture récente, en attente d'inspection, avec `date_statut`) et `Fermé changement d'exploitant` (changement de propriétaire). Sphères : restauration (cible), agroalimentaire (inclut transformateurs/distributeurs, pas seulement restaurants), construction/rénovation, immobilier/aménagement, entretien ménager, SST, sécurité physique, commerce de détail, **assurance/gestion des risques (meilleur proxy trouvé pour cette sphère à ce jour)**. Tier fort (`En traitement`) à moyen (`Fermé changement d'exploitant`). Portée : agglomération de Montréal seulement — complémentaire au RACJ, pas redondant (couvre les établissements sans permis d'alcool, invisibles au RACJ, et fournit une date que le RACJ n'a pas).

**5. OQLF — liste des entreprises certifiées — autorisation à demander avant activation, pas un chantier technique.** Livre le NEQ directement. Bulk par date de certification (liste déroulante depuis 1977, nouveau lot aux deux semaines). ⚠️ **Page ministérielle sans licence ouverte → régime par défaut, autorisation à demander avant activation** (formulaire en ligne, réponse ~15 jours ouvrables, redevance possible). Sphères : traduction/communications multilingues (cible, seul signal direct trouvé pour cette sphère), juridique/conformité réglementaire, gestion documentaire/archivage (signal le plus proche trouvé pour cette sphère, qui restait autrement sans solution), formation, technologie/TI, RH/marketing/commerce de détail en second rang. **Valeur unique : marqueur de seuil d'effectif daté** — la certification vise les entreprises de 25+ employés, confirmées à cette date par un organisme public, ce qu'aucune source active ne fournit aujourd'hui. Tier moyen — la certification marque la **fin** du processus de francisation, pas le moment du franchissement du seuil (délai possiblement de plusieurs années). **Deux listes sœurs à considérer aussi :** la liste des entreprises **non conformes** (besoin aigu, daté et non servi en traduction, tier fort comme signal de besoin mais faible comme signal de croissance — probablement la plus actionnable des trois pour cette sphère) et la liste des ententes particulières en vigueur (entreprises à forte composante étrangère/multilingue).

**6. AMF — bulletin hebdomadaire, placements sous dispense — bloqué, ne pas activer sans entente écrite.** Aurait été un excellent complément au RDPRM (RDPRM capte le financement par dette garantie, l'AMF aurait capté le financement par capitaux propres privés — les deux moitiés du signal financement). Sphères visées : comptabilité/finance/fiscalité (cible), juridique/conformité réglementaire, planification stratégique. **🚫 Conditions générales de l'AMF interdisent nommément d'"indexer dans une base de données" son contenu sans consentement écrit préalable** — pas une zone grise, une interdiction explicite. **Piste de contournement non vérifiée à explorer avant d'abandonner le signal :** la même déclaration (formulaire 45-106F1) est déposée dans SEDAR+ et publiée par les autres membres des ACVM — un régulateur d'une autre province pourrait la diffuser sous des conditions moins restrictives.

**Vérifiées et écartées :**
- **RPEVL/CTQ (véhicules lourds, Québec)** — outil de recherche par nom seulement (règle de vérification #1), écarté comme source de découverte. Réserve : pourrait servir d'enrichissement sur une entreprise déjà détectée (assurance, SST, logistique), même modèle que RDPRM mécanisme 2.
- **Registre des licences alimentaires de l'ACIA (fédéral)** — aurait été excellent (pancanadien, sphères #6/9/10/2), mais `apps.inspection.canada.ca` interdit l'accès automatisé par robots.txt. Deux voies non explorées : export manuel ponctuel, ou demande de données directe à l'ACIA.
- **MAPAQ (liste provinciale des permis)** — vérifié et écarté sous sa forme actuelle : accès automatisé interdit, couverture incomplète et officiellement documentée comme volontaire hors restauration/détail, régime de droit d'auteur par défaut. **Voie de contournement trouvée** : une demande d'accès à l'information de 2026 a produit publiquement la liste complète des titulaires en transformation alimentaire (XLSX) — utilisable comme socle de référence ponctuel via l'import manuel (section 9), pas comme veille continue.
- **BDIC (importateurs canadiens, ISED)** — instantané annuel seulement, biaisé vers les gros importateurs (80% du volume d'une catégorie), latence 1-2 ans, tier faible. Gardé en réserve, pas prioritaire.
- **Établissements alimentaires Nouvelle-Écosse** — vérifié en téléchargeant le vrai fichier (bulk, API Socrata). Champ `Permit Status` = `Awaiting Application` équivaut au signal montréalais. Limites sérieuses : aucune date, aucun identifiant d'entreprise, très bruité (mêle écoles/églises/légions/hôpitaux aux vraies entreprises), filtrage indispensable. **Conditions d'utilisation non vérifiées** (page bloque l'accès automatisé) — à lire manuellement avant activation, ne jamais présumer qu'une licence provinciale ouverte autorise le commercial par habitude. Valeur stratégique si activée : complète "contrats Nouvelle-Écosse" déjà actif, dans une province sans ancrage Piste.

**Sphères où rien de valable n'a été trouvé, à documenter honnêtement plutôt qu'à forcer une suggestion faible :**
- **Service à la clientèle/support technique** — aucun signal distinct; recommandation : règle de mots-clés sur Guichet-Emplois (déjà actif) plutôt qu'une nouvelle source.
- **Gestion documentaire/archivage** — seul l'angle OQLF (indirect); la Loi 25 crée une obligation universelle non discriminante.
- **Analytique d'affaires/gestion de données** — seulement les brevets CIPO, couverture très mince chez les PME.
- **Assurance/gestion des risques** — aucun signal direct; alimentée uniquement par dérivation d'autres signaux (nouvel établissement alimentaire, permis de construction, RDPRM) — à documenter comme sphère dérivée plutôt que de continuer à chercher une source qui n'existe probablement pas.
- **Chaîne d'approvisionnement/achats et import/export/douanes** — partiellement couvertes seulement, par des sources contraintes (ACIA bloquée, BDIC faible) — à reprendre plus tard.

### Recherche de sources Piste additionnelles — registres provinciaux (état des lieux)

Aucune nouvelle source Piste confirmée en dehors du REQ à ce jour. Table de référence complète des organismes réels par province/territoire, pour éviter de refaire cette recherche :

| Province/Territoire | Organisme réel | Type d'organisme | Accès |
|---|---|---|---|
| Québec | Registraire des entreprises (REQ) | Gouvernement | Gratuit, extrait en vrac — **source Piste confirmée** |
| Fédéral | Corporations Canada (ISED) | Gouvernement | Recherche gratuite, extrait en vrac séparé — déjà actif |
| Ontario | Ontario Business Registry (OBR), ServiceOntario | Gouvernement | Recherche gratuite; extrait en vrac à négocier; API réservée à des partenaires licenciés (ex. MinuteBox) — **piste la plus prometteuse, mais demande une démarche d'affaires, pas un chantier technique** |
| Colombie-Britannique | BC Registry Services, Service BC | Gouvernement | Payant à la recherche, pas d'extrait en vrac trouvé |
| Alberta | **Aucun accès direct au gouvernement** — uniquement via des agents de registre privés | **Privé** (agents indépendants multiples) | Frais de service non réglementés, varient par agent |
| Manitoba | Companies Office (Manitoba) | Gouvernement | À valider plus en détail |
| Saskatchewan | Corporate Registry, opéré par **Information Services Corporation (ISC)**, entreprise privée cotée en bourse (TSX : ISC), contrat exclusif depuis 2013 | **Privé** (une seule entreprise sous contrat de 20+ ans) | Recherche en ligne via leur plateforme |
| Nouvelle-Écosse | Registry of Joint Stock Companies (RJSC), Service Nova Scotia | Gouvernement | Distinct de la source "contrats Nouvelle-Écosse" déjà active (contrats publics, pas immatriculation) |
| Nouveau-Brunswick | Corporate Registry, Service New Brunswick (SNB) | Gouvernement | Payant, **et interdit explicitement les outils automatisés dans ses conditions d'utilisation — blocage légal, pas seulement un coût** |
| Île-du-Prince-Édouard | Corporate/Business Registry, Access PEI | Gouvernement | À valider plus en détail |
| Terre-Neuve-et-Labrador | Registry of Companies, Service NL | Gouvernement | À valider plus en détail |
| Yukon | Yukon Corporate Online Registry (YCOR) | Gouvernement | Recherche de base gratuite |
| Territoires du Nord-Ouest | Corporate Registries, ministère de la Justice (GTNO) | Gouvernement | À valider plus en détail |
| Nunavut | Registres juridiques, ministère de la Justice | Gouvernement | À valider plus en détail |

**Candidat Réflexion trouvé en cours de route (pas Piste) :** un outil tiers (Apify, développeur communautaire, "Scrape Sage") interroge la recherche fédérée gratuite du gouvernement (8 registres : fédéral + ON, QC, C.-B., AB, MB, SK, N.-É.) par nom d'entreprise — utile pour confirmer/enrichir une entreprise déjà détectée (renforcerait l'ancrage hors Québec), jamais pour découvrir de nouvelles entreprises en vrac. Deux réserves avant d'y toucher : conditions d'utilisation mentionnant des restrictions possibles sur l'usage commercial (à vérifier, cohérent avec la charte section 8/12), et un historique très mince (2 utilisateurs au total, outil très récent) — pas encore assez éprouvé pour s'y fier sans test.

**Reclassification confirmée — Corporations Canada est un Piste partiel pan-canadien, vérifié contre le code réel.** Un numéro de société fédéral permanent, valide dans les 10 provinces à la fois, contrairement à toute autre source trouvée dans cette recherche. **Deux limites structurelles honnêtes, pas des échecs :**
- **Tier moyen seulement, jamais fort.** Le seul signal réel produit est `changement_adresse` (même mécanique et même tier de confiance que l'équivalent REQ) — le registre fédéral n'expose pas de sous-structure par établissement, donc aucun équivalent au `nouvel_etablissement_secondaire` du REQ (tier fort) n'est possible ici, structurellement, pas par manque d'effort.
- **Couverture partielle, pas universelle.** N'ancre que les entreprises ayant choisi l'incorporation fédérale — pas l'ensemble des entreprises d'une province comme le REQ le fait obligatoirement au Québec. Reste utile dans les 9 provinces sans REQ équivalent, pour le sous-ensemble d'entreprises qui s'y qualifient.

**Précision importante sur le mécanisme, qui corrige une fausse impression :** ni le REQ ni Corporations Canada ne font de "découverte en vrac instantanée" au sens où une toute nouvelle incorporation deviendrait un signal immédiatement — les deux exigent qu'une entreprise soit déjà connue d'un import précédent avant qu'un changement (comme un changement d'adresse) puisse être détecté par différence. Une correction de calibration explicite (2026-08-31) a d'ailleurs corrigé le code d'origine, qui traitait à tort toute nouvelle incorporation comme un signal (~695 000 signaux au premier import) — cohérent avec le principe déjà établi pour le REQ ("la toute première immatriculation n'est pas une entreprise EN croissance").

**État réel en base : 0 signal à ce jour, attendu et non un échec.** Un seul run complet a eu lieu (2026-08-31), qui a peuplé le miroir local pour la première fois (694 844 corporations fédérales actives) — par construction (différence contre un état précédent), ce premier run ne peut produire aucun signal. Un deuxième run contre un fichier distant réellement modifié est nécessaire pour observer un vrai changement d'adresse détecté.

**Deuxième rôle distinct, à ne jamais confondre avec la production de signal :** `resolve_corp_federale_by_name` sert de porte de calibration pour `licences_toronto`/`licences_vancouver` — confirme qu'un nom détecté correspond à une corporation fédérale existante (`corporation_federale_correspondante`/`score_correspondance`), mais **n'est jamais un pivot de résolution de `Company`** (le NEQ reste le seul pivot). Les deux rôles cohabitent dans le même module mais répondent à des questions différentes.

**Écarté après vérification :** le numéro d'entreprise de l'ARC (BN) est pan-canadien et déjà utilisé comme identifiant dans plusieurs registres provinciaux, mais l'ARC ne publie aucun registre en vrac (confidentialité fiscale) — utile seulement pour confirmer une entreprise déjà trouvée ailleurs, jamais pour en découvrir de nouvelles. Même limite pour tout outil de recherche fédérée par nom, peu importe le nombre de provinces couvertes — la recherche par nom ne peut jamais remplacer la découverte en vrac.

**Écarté après vérification, principe structurel plutôt que technique — les agences de revenu (fédérale et provinciales) comme source Piste.** Idée testée : les ministères du revenu connaissent une entreprise dès son inscription à la taxe de vente (TPS/TVH, TVQ, etc.), potentiellement plus tôt et plus largement qu'un registre d'entreprises. **Écarté pour l'ARC (TPS/TVH), vérifié directement** : le registre de vérification TPS/TVH n'est qu'un outil de confirmation (exige déjà de connaître le nom ET une date de transaction, jamais de découverte en vrac), et ses propres conditions d'utilisation interdisent explicitement "toute reproduction commerciale des résultats" — un blocage légal direct, pas une zone grise. **Exception étroite trouvée, non applicable à notre besoin** : l'ARC publie une vraie liste en vrac pour la TPS/TVH simplifiée de l'économie numérique (nom, numéro d'entreprise, date d'inscription) — mais son champ d'application se limite aux entreprises numériques étrangères vendant à des consommateurs canadiens, pas aux PME générales visées par FALKYE. **Principe généralisé, à ne pas revérifier au cas par cas :** une agence de revenu existe pour percevoir l'impôt confidentiellement, pas pour publier un registre d'entreprises — contrairement à un registre des entreprises, conçu justement pour la divulgation publique. Ce principe s'applique probablement de la même façon à Revenu Québec (TVQ) et aux agences de revenu provinciales équivalentes (C.-B., Saskatchewan, Manitoba) — non vérifié individuellement, mais la même logique structurelle s'applique.
...
| Corporations Canada | Immatriculation fédérale — **Piste partiel confirmé** (numéro de société fédéral stable, tier moyen seulement — changement d'adresse — jamais fort). **N'ajoute rien au Québec** (REQ couvre déjà toute entreprise fédérale y opérant) — sa vraie valeur est dans les 9 autres provinces, seul point d'ancrage disponible à ce jour | ✓ | ✓ | ✓ | Gratuit | Actif, 0 signal à ce jour (un seul run de référence effectué, attendu) |
...
| RDPRM (mécanisme 1 — opérateur) | Enrichit le signal financement (garanties sur prêts), pipeline standard partagé | — | ✓ | ✓ | Nous payons, à l'unité (11$/nom, 4$/NIV) | Stub — aucun signal réel produit à ce jour (Phase 2) |
| RDPRM (mécanisme 2 — portail client) | Enrichissement scopé à l'instance du client uniquement, jamais partagé | — | ✓ | ✓ | Le client paie sa propre recherche à l'unité | Candidat — format du document jamais examiné, configuration par sphère à construire |
...
### Intégrations — jamais des sources, jamais dans le tableau ci-dessus

Une intégration est un point de **sortie** (FALKYE pousse ses résultats déjà détectés vers un système externe) — l'inverse d'une source, qui est un point d'**entrée** alimentant le moteur de croisement. Ne jamais les mélanger dans la même liste.

| Intégration | Rôle | Écho | Radar | Radar+ | Manière de payer | Statut |
|---|---|---|---|---|---|---|
| HubSpot | Intégration CRM (sortie des opportunités détectées) | — | ✓ | ✓ | Le client paie son propre compte | Actif, validation réelle en attente |
| Pipedrive | Intégration CRM (sortie des opportunités détectées) | — | ✓ | ✓ | Le client paie son propre compte | Actif, validation réelle en attente |
...
|---|---|---|---|
| **Écho** | Dès 29,99$/mois — **jamais gratuit**, seules les sources qu'il utilise le sont | Sources gratuites uniquement (l'ensemble du registre actuel) | Aucun |
| **Radar** | 89$/mois | Écho + sous-ensemble de sources payantes choisies par nous | Le même portail que Radar+, mais restreint aux sources qu'on propose, avec une couche de **paiement intégré** (l'utilisateur paie pour débloquer, nous gérons l'accès à la source) |
| **Radar+** | Dès 349$/mois | Radar + n'importe quelle source payante externe que l'utilisateur possède déjà | Le même portail, ouvert sans restriction, avec une couche de **gestion de clés API utilisateur** (l'utilisateur branche son propre accès, nous ne payons ni ne gérons la source) |
