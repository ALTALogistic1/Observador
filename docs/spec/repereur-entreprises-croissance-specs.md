...
Vue tableau de bord listant les dossiers cumulatifs sous forme de pastilles/cartes, réservée aux plans Radar et Radar+ (absente d'Sonar, dès 29,99$/mois — Sonar n'est pas un palier gratuit, seules les sources qu'il utilise le sont). Chaque carte affiche :
...
8. **Assistance à la configuration du profil par IA, à deux niveaux — seul usage de ML retenu dans tout le produit.** Contrairement au moteur de score (confiance/pertinence), qui reste en règles simples et documentées pour rester explicable, ceci touche uniquement l'accueil du profil : l'utilisateur décrit en texte libre ce qu'il fait/offre, l'IA propose les sphères de besoin et mots-clés correspondants. **Le résultat est toujours une proposition que l'utilisateur confirme ou corrige — jamais une classification silencieuse qui affecte ses résultats sans qu'il la voie.**
   ...
   - **Gating par plan à trancher :** un appel à un modèle de langage a un coût réel par utilisation — à réserver aux paliers payants (Sonar n'étant lui-même jamais gratuit, dès 29,99$/mois, ça reste à trancher si Sonar a accès à la version complète ou seulement au niveau 1, moins coûteux).
...
Un prospect qui sonarue à l'une de ces vérifications est **exclu silencieusement**, pas présenté avec un avertissement — la promesse du produit repose sur la fiabilité de ce qui est montré, pas sur la quantité.
...
|---|---|---|---|
| **Sonar** | Dès 29,99$/mois — **jamais gratuit**, seules les sources qu'il utilise le sont | Sources gratuites uniquement (l'ensemble du registre actuel) | Aucun |
| **Radar** | 89$/mois | Sonar + sous-ensemble de sources payantes choisies par nous | Le même portail que Radar+, mais restreint aux sources qu'on propose, avec une couche de **paiement intégré** (l'utilisateur paie pour débloquer, nous gérons l'accès à la source) |
