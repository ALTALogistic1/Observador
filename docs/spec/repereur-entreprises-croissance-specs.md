...
**Limite honnête restante, non résolue :** le chemin hors-profil bout-en-bout (bonus/redirection sur une vraie notification Radar+) n'est validé que par tests unitaires — aucune donnée Radar+ réelle en base pour l'exercer en direct, et aucune n'a été fabriquée pour simuler une validation. À combler soit par un vrai client Radar+, soit par un scénario de bout en bout contre une base temporaire (même méthode que le scénario d'authentification déjà validé ainsi).
