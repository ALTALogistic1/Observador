...
| D60 | La LONGUEUR MINIMALE d'une forme au scorage — son échelle, et ce qu'on appelle « longueur » | Chantier 3+4 §12bis; `outils/longueur_des_formes.py` | ⬜ **Déclencheur : avant toute nouvelle écriture de NEQ.** *Rien ne borne aujourd'hui la longueur d'une forme du registre au scorage.* ⚠️ **`WRatio` rend au moins 90,0 quand un nom du registre est entièrement contenu dans le nom détecté, et DEUX LETTRES suffisent** — `« LA »` a scoré **95,0** sur `#5476`. *Une valeur, donc une échelle, donc Alexandre* — **qui la posera avec Claude, et ne veut aucune valeur posée d'avance pour débloquer.** ⚠️ **Et une SOUS-QUESTION vient avec** : `« L.A. »` se normalise en `« l a »` — **3 caractères, 2 lettres, 2 mots**. *Une borne de 3 caractères retirerait `« LA »` et garderait `« L.A. »`, qui est la même porte.* **`outils/longueur_des_formes.py` rend la distribution, le coût de chaque borne et les trois mesures; il ne propose rien.** |
| D61 | Au-delà de deux prétendants, le DÉTENTEUR garde-t-il son NEQ ? | Chantier 3+4 §12bis; `falkye/resolution.py::garde_des_pretendants` | ⬜ **Déclencheur : la première fois qu'un journal montre un 3e prétendant en production.** *La garde est descendue dans la résolution le 23 septembre, mais **sa forme a changé en descendant** : la passe par lot refuse à TOUS, la résolution refuse au NOUVEAU VENU.* ⚠️ **`Company.neq` est UNIQUE** : le partage n'est pas une issue, et retirer le NEQ au détenteur serait **une écriture qui EFFACE une identité**, déclenchée par l'arrivée d'un signal. **Le rang est donc seulement ÉCRIT au journal.** *Deux directions : laisser le premier arrivé le garder, ou révoquer au-delà de deux — et la seconde est destructrice.* |
**60 entrées, 15 tranchées.** *Compte relu dans le tableau le 22 septembre 2026 — il annonçait
