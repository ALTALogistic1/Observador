...
| D4 | Prix du palier supérieur | Spéc. 12.1 *(seul domicile des prix depuis D35)* | ⬜ *à dater — et le « à dater » n'a lui-même jamais été daté.* **Constaté le 11 septembre 2026 :** le renvoi de cette entrée pointait vers **spéc. §15**, qui ne porte plus la table — elle est en **12.1** *(corrigé)*; la table était **recopiée dans trois documents** dont **une seule copie portait ce renvoi** *(réglé par D35 : les prix ne vivent plus qu'en 12.1)*; et **aucune copie ne portait de date**, ni de fixation ni de dernière révision *(toujours vrai)*. **Ce qui manque pour clore :** depuis quand le « à réviser » est posé, et **la condition qui le lève** — une mention de révision sans déclencheur est une décision non tranchée, et une décision non tranchée disparaît *(charte §15, faille I, journal cas 15)*. |
...
- ⬜ Chemin d'import du miroir — deux allers-retours par ligne, mesuré. **Et le pic mémoire de 3 535 Mo
  ...
  n'était pas le chemin du jour, et 3 535 Mo sur 8 Go passe. Le tranchage par `limit`, lui, a été écarté
  — pas d'`offset`, donc une deuxième tranche repartirait de la ligne 1 et coûterait plus qu'un bloc.*
