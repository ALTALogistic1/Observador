...
| D14 | Le repli par sous-chaîne : borner ou retirer? | Chantier 2, instrument de coût | ⬜ **toujours ouverte, et le cycle du 11 septembre ne l'a pas tranchée : il n'a rien résolu.** *Zéro sur les trois chemins, `~0` ligne dérivée, repli par sous-chaîne non emprunté — la deuxième fois, après le 10 septembre.* **Ce que le 11 septembre a appris, et ce n'est pas rien :** le miroir n'était plus en cause *(D15)*, et pourtant la mesure n'a pas eu lieu — **les trois compteurs ne s'incrémentent que sur un signal NEUF**, et un cycle en régime n'en ingère pas assez. *Le déclencheur reste donc entier : un cycle qui ingère du NEUF (première édition REQ d'un trimestre neuf, source réactivée), pas seulement un cycle qui en aurait les moyens.* **Le multiplicateur, inchangé :** chaque résolution qui échoue en préfixe balaie **8 396 lignes facturées**. ⚠️ **Ce qui a changé, c'est le rang de la question.** Le 11 septembre a mesuré **411 963 777 lectures pour ce cycle à zéro résolution** : le repli par sous-chaîne n'est plus le premier consommateur de la base, et il ne l'était peut-être pas le 8 septembre non plus *(D37, journal cas 33)*. **D14 reste à trancher — elle n'est plus la question qui décide du quota.** |
...
| D37 | Le chargement des signaux par entreprise — quelle correction | `falkye/engine.py::generer_notifications`, `falkye/expansion_interprovinciale.py::detecter_expansions`, `falkye/models/signal.py` | ⬜ **avant le prochain cycle, et le déclencheur est le coût lui-même : 412 M de lectures à chaque passage.** *Constaté le 11 septembre 2026, au compteur de l'hébergeur et dans ses requêtes les plus coûteuses (journal, cas 33).* **Ce qui est établi, mesuré :** `SELECT … FROM signals WHERE ? = signals.company_id` — **17 800 lignes lues en moyenne, 23 100 appels**, soit ≈ 411 M, l'écart mesuré à moins d'un pour cent. **Trois faits qui se cumulent :** *(a)* `signals.company_id` est une clé étrangère **sans index** — en SQLite une clé étrangère n'en crée aucun, donc chaque appel balaie la table; *(b)* la relation `Company.signals` se charge **une entreprise à la fois**; *(c)* elle est parcourue dans **deux** balayages complets de `companies` — `detecter_expansions` puis `generer_notifications` — d'où 23 100 ≈ 2 × 11 550. **Ce qui reste à trancher, et aucune option ne se prend en passant :** poser l'index, charger les signaux en un coup pour toutes les entreprises du passage, ou revoir les deux balayages complets eux-mêmes — *les trois ne coûtent pas la même chose et ne couvrent pas le même périmètre : l'index seul laisse 23 100 allers-retours, le chargement groupé laisse les deux balayages, et la restructuration touche le chemin de la détection.* ⚠️ **Même forme, non chiffrée :** `evaluer_pour_company` interroge `liens_interprovinciaux` **une fois par entreprise** — invisible aujourd'hui parce que la table est petite, et elle ne le restera pas. |
| D38 | Redescendre du palier de quota supérieur | Chantier 29, chantier 2 §1ter | 🔴 **NON — tranché le 11 septembre 2026, et sur une mesure.** *L'entrée attendait « la ventilation du coût sur un cycle complet en régime » (chantier 29). Elle existe : un cycle ordinaire, huit sources en succès, **411 963 777 lectures au compteur de l'hébergeur** — relevé avant et après, donc une mesure et non une dérivation.* **Le palier ne descend pas tant que ce chiffre tient.** *Ce n'est pas un report : c'est un non motivé, qui se rouvre à une condition nommée — un cycle mesuré au compteur sous le quota du palier visé, D37 corrigée.* ⚠️ **Ne pas rouvrir sur le rapport de coût seul** : il ne compte que les trois chemins de résolution, et il affichait zéro ce jour-là *(journal, cas 33)*. |

**Trente-sept entrées, sept tranchées.** *Compte relu dans le tableau le 11 septembre 2026 — la phrase
