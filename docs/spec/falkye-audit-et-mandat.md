...
| D4 | Prix du palier supérieur | Spéc. 12.1 *(seul domicile des prix depuis D35)* | ⬜ *à dater — et le « à dater » n'a lui-même jamais été daté.* **Constaté le 11 septembre 2026 :** le renvoi de cette entrée pointait vers **spéc. §15**, qui ne porte plus la table — elle est en **12.1** *(corrigé)*; la table était **recopiée dans trois documents** dont **une seule copie portait ce renvoi** *(réglé par D35 : les prix ne vivent plus qu'en 12.1)*; et **aucune copie ne portait de date**, ni de fixation ni de dernière révision *(toujours vrai)*. **Ce qui manque pour clore :** depuis quand le « à réviser » est posé, et **la condition qui le lève** — une mention de révision sans déclencheur est une décision non tranchée, et une décision non tranchée disparaît *(charte §15, faille I, journal cas 15)*. **⚠️ Laissée ouverte DÉLIBÉRÉMENT le 11 septembre 2026, et ce n'est pas un oubli.** *Les prix n'ont aucune importance pour la réalisation du produit : ils sont indicatifs, et ils le resteront **jusqu'à ce qu'il y ait un client**. Ni date de fixation ni condition de levée ne seront fournies avant.* **Ne pas rouvrir ce point, ne pas proposer de date, ne pas déduire la condition** — l'inventer serait la décision que personne n'a prise *(charte, règle 5)*. |
| D5 | Autorisation OQLF | Spéc. §15, §4.6 | ⬜ *à dater* — ✏️ **le motif de cette décision a changé le 14 septembre 2026, et il faut le relire avant de la trancher.** *L'OQLF était l'unique voie vers un effectif : seuil de 25+ employés, daté, par un organisme public, que la spéc. 8.2 classe en taille **mesurée**. Son blocage n'est ni technique ni budgétaire — c'est une demande d'autorisation au gouvernement du Québec.* **Mais le REQ porte une bande d'effectifs, gratuitement, dans un fichier déjà importé** *(journal, cas 38)* : `COD_INTVAL_EMPLO_QUE`, rempli à 100 %, ~47 % de bandes exploitables. **L'OQLF reste plus précise et mieux datée; elle n'est plus la seule.** *Et deux simulations de portrait sur neuf reposaient sur elle — le catalogue la classe cinquième à cause de D5, les simulations suggéraient qu'elle devrait être première. À trancher, pas à faire.* |
...
| D22 | Seuil de publication — à partir de quelle case un dossier devient une opportunité présentée | Mandat chantier 21, §1 | ⬜ *à dater* — **distinct du curseur de sensibilité de l'utilisateur**, les fusionner lui retirerait la possibilité d'être plus sélectif  ⚠️ **S'y rattache depuis le 23 septembre : la RÈGLE DE FRAÎCHEUR** *(chantier 21, §1)* — *« on ne voudrait pas proposer un prospect dont l'activité économique date de plus d'un an »*. **Disqualifier n'est pas moduler** : la modulation du grade est D24. *Ce qui la borne, et qui n'est écrit nulle part : la profondeur d'historique de chaque source.* |
...
| D35 | Où vit la table des paliers, et laquelle des trois copies fait foi | Charte §5, spéc. 12.1, `strategie-et-personas.md` | ✅ **Tranché le 11 septembre 2026 : la spécification fait foi, section 12.1.** *Elle porte l'état à jour, c'est son rôle.* **La charte garde la structure des plans comme principe** — trois paliers, ce que chacun sert — **et cesse de porter les prix**; `strategie-et-personas.md` y renvoie. **Un prix vit à un seul endroit.** *Le motif : **un principe ne se périme pas, un prix oui**, et une seule des deux natures appartient au document qui tranche en cas de contradiction. Les deux lectures se défendaient — c'est la nature de l'objet qui a départagé, pas le rang des documents.* **Ce que la copie avait déjà coûté :** les trois versions divergeaient, et **la charte était la moins complète** — elle omettait le « jamais gratuit » du palier d'entrée, **l'affirmation exacte que le README avait contredite deux jours plus tôt**. *Règle appliquée : une phrase qui a une source de vérité ailleurs n'a rien à faire là (guide d'ingénierie). La datation de la table reste ouverte à D4.* |
| D36 | L'état de diff est validé AVANT les signaux qu'il a produits | `falkye/diff_engine.py` (`_appliquer_diff`), `falkye/engine.py::ingest_source` | ⬜ **avant le premier cycle qu'on envisagerait d'interrompre.** *Constaté le 11 septembre 2026 en répondant à « peut-on arrêter un cycle qui dérape ».* **Le mécanisme.** `executer_diff` applique l'état — `EtatLigneSource`, base des miroirs — **avant** que le connecteur émette ses signaux, et ne valide jamais : il ne fait que `flush()`. C'est `ingest_source` qui valide, **une fois par signal neuf**. **Donc au premier signal validé, tout l'état de diff de la source est durable alors qu'un seul signal existe.** Une interruption après ce point laisse l'état avancé sur des changements dont les signaux n'ont jamais été écrits, et **la relance ne les reverra pas** — elle diffère contre l'état déjà avancé. *Aggravant : deux bases, donc `commit()` valide les deux liaisons l'une après l'autre, jamais atomiquement.* **Ce que ça ne coûte pas :** la donnée n'est pas perdue du monde, `_archiver_snapshot` garde les lignes brutes — mais les retrouver demande de restaurer l'état précédent à la main. *Ce n'est pas une reprise, c'est une réparation.* **Deux corrections possibles, et le choix appartient à Alexandre :** *(a)* valider l'état de diff **après** le dernier signal de la source — la forme juste, mais elle rouvre la panne que la validation par signal avait fermée (une source qui tombe perdrait tout); *(b)* garder la validation par signal et **journaliser le point d'avancement** pour qu'une reprise sache quoi rejouer. *Aucune ne se tranche en passant : la première échange une classe de perte contre une autre.* **La conséquence opérationnelle, et c'est elle qui décide de la suite : on peut arrêter un cycle, mais pas sans perdre la détection du reste d'une source.** *Précision qui compte — **l'interruption, elle, laisse une trace** : le `SourceRunLog` reste à `en_cours` et le mécanisme d'exécution interrompue du chantier 2 le relève. **C'est la PERTE qui n'en laisse aucune** : rien ne dit quels changements ont été avalés par l'état avancé, et le cycle suivant est muet plutôt que fautif.* **Tant que D36 est ouverte, arrêter un cycle est un arbitrage humain** — la perte de détection d'une source sur un cycle, rattrapable au prochain passage du jeu de données, contre un quota vidé. *Marche et zones d'arrêt : `docs/DEPLOIEMENT.md`, « Arrêter un cycle en cours ».* **🟡 RISQUE ACCEPTÉ le 11 septembre 2026 — pour UN cycle, et la portée fait partie de la décision.** *Accepté pour un **cycle de mesure**, lancé avec `--sans-livraison`; **pas pour un cycle avec livraison**.* Le motif de l'acceptation : le pire cas est la perte de détection d'**une** source sur **un** cycle, **réel mais borné**, et rattrapable au prochain passage du jeu de données. **⚠️ L'entrée ne se ferme pas là-dessus, et c'est délibéré : une acceptation bornée n'est pas une décision.** *La question se repose **avant l'activation du minuteur*** — un cycle automatique livre, personne ne le regarde partir, et personne ne sait qu'une source a été perdue. **Le correctif retenu à terme, pour que l'arbitrage ne se rouvre pas depuis zéro : journaliser le point d'avancement**, afin qu'une reprise sache quoi rejouer. *Il coûte peu et **retire la perte silencieuse sans échanger une classe de perte contre une autre***, ce que ferait la validation après le dernier signal — celle-là rouvrirait la panne que la validation par signal avait fermée (une source qui tombe perdrait tout). **L'option écartée est nommée pour qu'on ne la repropose pas comme neuve.** **🔁 LA QUESTION SE REPOSE — 11 septembre 2026, le cycle de mesure a eu lieu et l'acceptation bornée est épuisée.** *Ce que change un cycle QUI LIVRE, et les trois points ne se valent pas.* **(1) Personne ne le regarde partir.** Le cycle de mesure a été lancé à la main, journal sous les yeux : une interruption se voyait à la seconde. Un cycle de minuteur part seul, et **le seul témoin de la perte serait un `SourceRunLog` resté `en_cours`** — qui dit qu'une source a été interrompue, **jamais ce qui a été avalé par l'état de diff avancé**. **(2) Le délai est huit fois plus court** — 5 400 s contre 43 200 s : la marge est de 3,1 sur un régime de 27 min, confortable, mais c'est l'unité qui livre qui porte le risque d'être tuée en cours de source. **(3) Et c'est le point qui décide : la perte devient une AFFIRMATION AU CLIENT.** Sans livraison, une détection perdue est un trou interne, rattrapable au passage suivant. Avec livraison, le résumé part en disant qu'il n'y avait rien — *le silence cesse d'être une absence et devient une phrase adressée à quelqu'un*, sur un produit dont la faille A dit qu'un abonné qui ne reçoit rien ne revient pas. **Le correctif retenu ne change pas : journaliser le point d'avancement.** *Il coûte peu, et il retire la seule chose qui rend l'arbitrage nécessaire — le fait que la perte ne laisse aucune trace.* **Trancher, c'est soit le construire, soit accepter explicitement une perte silencieuse pouvant partir chez un client. La seconde n'a jamais été acceptée, et ne l'a pas été le 11 septembre non plus : l'acceptation portait sur un cycle SANS livraison.** **🔁 REPOSÉE ET RÉDUITE — 14 septembre 2026, avant l'activation, comme l'entrée l'exigeait.** *Trois faits neufs, et le troisième dissout le verrou.* **(1) La portée est d'UNE source sur cinq, pas de toutes.** Le mécanisme exige `executer_diff`, qui n'est emprunté que par les sources en `type_ingestion: instantane` — **et `req` est la seule source active qui en soit**. **(2) Pour les autres, rien ne s'avance.** `since` vaut `maintenant − 30 jours`, recalculé à chaque cycle (`engine.py`), **jamais dérivé de la dernière réussite** : une interruption ne fait avancer aucune fenêtre, et le cycle suivant relit la même. *Et les deux connecteurs fédéraux sont allés plus loin le 14 septembre — ils dédupliquent contre les RÉFÉRENCES EN BASE, pas contre un état : une référence non écrite est par construction inconnue au passage suivant, donc réémise.* **(3) ⚠️ ET LE CYCLE N'INGÈRE PAS LE REQ.** `ingest_all_active_sources` **exclut les sources en `methode_acces: import_manuel`**, dont `req` et `rdprm`. **Aucune source du cycle ne passe par le moteur de diff**, et l'absence totale de répertoire d'archive sur l'hôte l'atteste *(journal, cas 39)*. **Conséquence : la perte silencieuse de D36 ne peut pas se produire sur le chemin du minuteur.** *Elle reste entièrement vraie pour l'import manuel du REQ.* **✅ D36 N'EST DONC PLUS UN VERROU À L'ACTIVATION** — c'est ce qui a permis de fermer le chantier 29 le 14 septembre. **Elle n'est pas close pour autant** : le jour où un import du REQ tournera, un arrêt en cours de source laissera l'état avancé sur des changements dont les signaux n'ont jamais été écrits. ⚠️ **Et le correctif envisagé ne tient pas à son propre dimensionnement.** *Persister la liste de travail du diff avait été proposé comme « borné par les seuils de quarantaine » : c'est faux.* `req.seuils_quarantaine` est **nul**, donc les seuils par défaut — apparitions à **50 % ET 500 absolues, les deux ensemble**. **Sur 2,7 M de lignes, 50 % font 1,35 million**, et l'absolu ne déclenche jamais seul : *un diff accepté peut porter plus d'un million d'apparitions.* **La piste qui reste : appliquer l'état ET émettre les signaux PAR LOTS ALIGNÉS**, de sorte que l'état n'avance jamais au-delà de ce qui est durable — aucun journal, aucune liste, moins d'écritures. *Le prix est ailleurs : `apres_diff_accepte` est aujourd'hui invoqué exactement UNE fois, et c'est une garantie du chantier 1 — la découper est un changement de contrat, pas un ajout.* **Rien n'est construit : la prémisse qui rendait ce travail urgent a disparu.** |
| D12 | Seuils d'alerte de dérive | Mandat chantier 2, livrable 6 | ⬜ **avant clôture du chantier 2** |
| D17 | Seuils de quarantaine par défaut | Mandat chantier 1, livrable 6; chantier 3+4 §17bis, étape 2 | ⬜ *à dater* — **même forme que D12, jamais une constante enfouie**. ⚠️ **ENRICHIE le 23 septembre : le REQ n'a AUCUN seuil propre**, donc les défauts s'appliquent — *apparitions à **50 % ET 500 absolues, les deux ensemble***. **Sur 2,7 M de lignes, 50 % font 1,35 million**, et l'absolu ne déclenche jamais seul : *un diff accepté peut porter plus d'un million d'apparitions* **(chiffrage complet dans D36)**. ⚠️ *Distinct de **D58**, le plancher de remplissage par fichier : deux échelles, deux objets.* **Déclencheur : avant l'import du REQ** — étape 2 du bloc de l'import. |
...
| D14 | Le repli par sous-chaîne : borner ou retirer? | Chantier 2, instrument de coût | ✅ **CLOSE le 11 septembre 2026 — ni bornée ni retirée, et le motif est un chiffre.** *Le repli n'a été emprunté AUCUNE des trois fois où on a mesuré* — 10, 11 septembre matin, 11 septembre soir — **et le cycle entier coûte désormais 120 311 lectures** *(mesuré au compteur de l'hébergeur, après l'index de D37)*. **Il ne coûte rien parce qu'il ne sert pas.** Le borner reviendrait à régler un mécanisme qui ne s'exécute jamais; le retirer coûterait un chemin de repli le jour où la résolution en aura besoin. *Le multiplicateur reste vrai et reste sans objet : 8 396 lignes facturées par résolution qui échoue en préfixe.* **⚠️ Ce qui la rouvrirait, et il faut que ce soit écrit** : **un cycle qui résout réellement des entreprises** — un premier import REQ d'un trimestre neuf, une source réactivée, une population qui grandit. *Alors le nombre d'appels cesse d'être zéro et la question redevient celle qu'elle était.* **La fermer sans cette condition serait conclure d'une absence de mesure à une absence de coût — exactement le défaut du cas 33.** |
| D15 | Le vide du 8 septembre — miroir absent ou appariement? | Journal cas 10, faille D | ✅ **Tranchée le 11 septembre 2026 : c'est l'APPARIEMENT.** Le miroir est chargé, complet, avec le correctif du 6 septembre déployé. **Artefact — lu sur l'hôte :** `2 730 146 entrées, dont 1 998 906 avec ville (73,2 %)`. *Attendu par `docs/MIROIRS.md` : ~2,7 M et ~66 % avec ville; un taux autour de 7 % aurait dit que le correctif n'était pas déployé.* **Donc les deux explications du corpus ne se partageaient pas le vide : le mur du miroir absent était tombé, et celui qui reste est le taux de vérification.** *La lecture « deux murs successifs » proposée à la faille D est confirmée pour le premier, et le second est seul en cause aujourd'hui.* **Ce que ça débloque :** un cycle qui résout a maintenant de quoi résoudre — la mesure de D14 est prenable, ce qu'elle n'était pas le 10 septembre. *Mesure faite avant le cycle plutôt qu'après, au titre de la règle 3 de la charte : quelques secondes contre un cycle entier payé pour réapprendre que le miroir n'était pas là.* |
| D37 | Le chargement des signaux par entreprise — quelle correction | `falkye/models/signal.py`, `falkye/engine.py`, `falkye/expansion_interprovinciale.py` | ✅ **TRANCHÉE le 11 septembre 2026 : l'index, et lui seul.** **Mesure, au compteur de l'hébergeur, avant et après :** **411 963 777 → 120 311 lectures** pour un cycle comparable — **divisé par 3 424**. *La requête qui dominait les requêtes les plus coûteuses n'y figure plus; celle qui la remplace lit 2 lignes par appel au lieu de 17 800.* **Ce que le correctif a coûté, une fois : ~17 824 écritures facturées** *(écart des compteurs d'écriture entre les deux cycles — la construction de l'index sur 17 822 lignes)*. Deux index posés : `ix_signals_company_id` et `ix_liens_interprovinciaux_company_id_b`, vérifiés au PLAN et non à la réussite du `CREATE INDEX`. **⛔ Les deux autres corrections sont ÉCARTÉES, et le motif est le chiffre** *(décision d'Alexandre)* : le **chargement par lot** et la **suppression de la double passe** rendraient la forme juste, mais **à 120 311 lectures par cycle elles ne rachètent plus rien.** *La forme reste imparfaite; elle ne coûte plus.* **⚠️ La raison tombe si la population grandit** — le coût reste **proportionnel à la base entière**, pas à ce qui compte : à dix fois plus d'entreprises, c'est dix fois plus de lectures, et la question se rouvre d'elle-même. *La conception qui produit cette proportionnalité est D39, et elle, elle reste ouverte.* |
| D38 | Redescendre du palier de quota supérieur | Chantier 29, chantier 2 §1ter | 🟡 **ROUVERTE le 11 septembre 2026 — le motif du « non » est tombé, et ce n'est pas un revirement : c'est le même critère appliqué à un nouveau chiffre.** *Le « non » du matin tenait à 411 963 777 lectures par cycle. Après l'index (D37) : **120 311**, soit ~521 000 par mois à cadence hebdomadaire.* **La lecture n'est plus le facteur qui décide** — *sur la fiche du palier Developer lue le 11 septembre 2026, ~0,02 % du quota mensuel de lectures, et les 28 écritures par cycle sont sans commune mesure avec le sien.* **⚠️ Ce qui décide maintenant est ailleurs, et il faut le trancher explicitement : la FENÊTRE DE RESTAURATION.** Descendre la raccourcit *(voir D13 — on ne sait toujours pas ce qu'elle fait en pratique, faute d'avoir eu quelque chose d'assez vieux à restaurer)*. **Donc la question n'est plus « le quota suffit-il » mais « combien de jours de retour en arrière veut-on payer ».** *Les chiffres des paliers ne sont pas recopiés ici : ils vivent à la fiche de l'hébergeur, et une table de prix recopiée se périme sans le dire (chantier 29). Ce qui est inscrit ici est le RAPPORT mesuré et sa date.* |
| D39 | Le départage des entreprises se fait APRÈS le chargement, pas avant | Chantier 22 (la lecture), `falkye/engine.py::generer_notifications` | ⬜ **Conception à revoir, pas défaut à réparer — inscrite le 11 septembre 2026 sur constat d'Alexandre.** *Le moteur charge les signaux de toutes les entreprises, puis trie.* **C'est ce qui rend le coût proportionnel à la base entière plutôt qu'à ce qui compte.** **Le corollaire, et c'est lui qui décide de la suite : enrichir le départage ne réduira le coût que si le tri PRÉCÈDE le chargement.** *Un meilleur tri appliqué après ne fait qu'écarter plus tard ce qu'on a déjà payé.* **Pourquoi elle n'est pas urgente et pourquoi elle ne se ferme pas** : après D37 le cycle coûte 120 311 lectures, donc rien ne presse — **mais la proportionnalité, elle, n'a pas bougé**, et c'est elle qui rouvrirait D37. *Elle appartient au chantier 22 parce que « ce qu'on retient de lire » est une question de lecture, pas d'optimisation.* |
| D40 | Les clés étrangères sans index qui restent | `falkye/models/notification.py` | ⬜ **au premier cycle qui produit des notifications.** *Trouvé le 11 septembre en cherchant les jumeaux de D37 :* `notifications.company_id` et `notification_signals.notification_id` sont des clés étrangères **sans index** — en SQLite une clé étrangère n'en crée aucun — et `_signaux_deja_couverts` les interroge **une fois par entreprise CANDIDATE**. **Délibérément NON posés** : ce chemin ne s'exécute que pour une entreprise dont un signal a déjà matché un profil, donc **zéro fois dans le régime actuel**. *Non mesuré ⇒ non posé — poser un index sur un chemin qu'on n'a pas vu tourner, c'est construire le mécanisme avant son défaut (D31).* **Le déclencheur est donc le premier cycle qui produit des notifications**, et le coût du report est un déploiement. |
| D41 | Le filtre territorial n'est déclaré que par UNE source sur huit | `falkye/registry/sources.yaml`, `falkye/territoire.py` | ⬜ **avant d'ajouter une source pancanadienne, et au plus tard avec D27/D28.** *Lu le 11 septembre 2026 :* **seule `eimt` déclare un `territoire`.** Les autres ont `territoire: null`, et `appartient()` **retient tout** quand la liste est vide — *à dessein, et la raison est bonne : « une valeur absente est retenue, jamais rejetée », sinon une source qui ne sait pas renseigner la région verrait tout son contenu disparaître.* **Mais la conséquence l'est moins : le « filtre territorial actif » filtre exactement une source**, et quatre des sept autres sont pancanadiennes — elles écrivent dans `companies` sans borne de territoire, **alors que tout ce qui touche un territoire hors Québec est en veilleuse depuis le 4 septembre**. *Ce n'est pas un défaut à corriger en passant : déclarer un territoire sur une source qui ne renseigne pas la région la ferait taire entièrement.* **Ce qui manque pour trancher : savoir ce que chaque source SAIT dire de la région, avant de décider ce qu'on lui demande.** ✏️ **Précision mesurée le 14 septembre 2026, et elle vaut pour toute lecture de cette entrée : le filtre est INERTE pour le SEAO, deux fois plutôt qu'une.** *(a)* `seao` ne déclare **aucun** `territoire` — son `region: Québec` est du texte libre, explicitement distinct du filtre (`registry/loader.py`). *(b)* Le connecteur **ne pose jamais `region`** sur le `RawSignal`, et `appartient(None, …)` **retient** par principe — *« une valeur absente est retenue, jamais rejetée »*. **Zéro attribution SEAO écartée par le territoire, sur 36 477 sans signal** *(journal, cas 40)*. |
| D42 | Globe and Mail Top Growing et Deloitte Fast 50 | Spéc. 8.2, stratégie §8 | ✅ **TRANCHÉE le 11 septembre 2026 : les deux en veilleuse** *(`statut: en_pause`, code, tests et état accumulé conservés, reprise sans run de référence — même forme que les trois sources hors Québec du 4 septembre)*. **Le motif : elles visent la mauvaise population.** *Spéc. 8.2 — sans normalisation, tout signal de volume favorise mécaniquement les grandes entreprises. Un palmarès national liste des entreprises qui ont FINI de croître, pas la clientèle du produit.* **⚠️ CE QUI A ÉTÉ PESÉ, et qui ne doit pas rouvrir la décision tout seul** : ces deux sources sont **deux des quatre seulement** qui portent la **ville** jusqu'au dossier, et la ville vaut +5 au score de résolution NEQ en plus de départager deux entrées du REQ — **elles agissaient donc sur le taux d'appariement, le mur qui reste après D15**, et enrichissaient le dossier mieux que le SEAO qui est au cœur du produit. **Les deux rôles d'une source — le signal qu'elle produit, et ce qu'elle ajoute au dossier une fois le signal détecté — pointaient ici en sens CONTRAIRE. Le signal ne sert pas, l'enrichissement servait, et c'est le signal qui a décidé.** *Ce qui a déjà été apporté reste en base; ce qu'on perd est l'enrichissement FUTUR, sur une population qu'on ne sert pas. Rouvrir en invoquant l'enrichissement seul serait rejouer un argument déjà pesé et écarté.* **Confirmé par un autre chemin le soir même** : le classement Deloitte a rendu un 403 au cycle de mesure — le document n'est plus accessible. *La réserve vit aussi dans la note datée de `falkye/registry/sources.yaml`, là où on la lira en voyant `en_pause`.* |
| D43 | Le donneur d'ouvrage comme ENTITÉ du produit | Relevé SEAO du 13 septembre 2026, faille C | ⬜ **avec D27/D28/D29, jamais avant.** *Le SEAO porte DEUX prospects par avis, et le corpus n'en décrit qu'un.* **Le donneur d'ouvrage est le prospect DIRECT** — il entreprend quelque chose, la classification dit quoi, `buyer.id` est présent à 100 %. **Le fournisseur retenu est un prospect CONDITIONNEL** : un contrat décroché crée des besoins (embauche, équipement, assurance, cautionnement, fonds de roulement) **mais seulement quand il sort de l'ordinaire pour lui** — premier contrat, ou disproportion par rapport à la bande d'effectifs. ⚠️ **État du code : le connecteur ne crée une entreprise que pour le FOURNISSEUR.** Le donneur d'ouvrage **n'existe comme entité nulle part** — son nom vit dans `Signal.champs`, lu seulement par le message de premier contact; son identifiant est capté depuis le 14 septembre, pour le jour où il servira. ✏️ **Correction du 13 septembre, avant écriture au corpus (Alexandre) : « le donneur d'ouvrage est exploitable maintenant » est FAUX.** *Le faire exister est un changement de modèle, pas un réglage. Les deux prospects attendent donc chacun quelque chose : le fournisseur attend l'appariement, le donneur attend d'exister.* **Ce qu'il faudrait pour trancher** : un registre officiel des entités publiques (D27), une règle de classement avec réponse par défaut (D28), l'identifiant qui fait foi (D29), **et une identité interne distincte du NEQ** — exactement ce que les chantiers 3 et 4 construisent. ⚠️ **Et la population n'est pas homogène, vérifié à la source du gouvernement du Québec** : l'obligation de publier au SEAO ne pèse que sur les **organismes publics**, mais sociétés d'État à vocation commerciale, OSBL et entreprises privées peuvent l'utiliser volontairement. **Un donneur d'ouvrage n'est donc pas « public » par construction**, et une règle qui le présumerait se tromperait en silence. *`outils/donneurs_douvrage.py` compte les donneurs distincts et REFUSE d'en attribuer le type — ce serait D28, devinée.* |
| D44 | Ce qu'un profil peut DÉCLARER — les publics non commerciaux | Spéc. §4, simulations des 13-14 septembre 2026 | ⬜ **avant le chantier 25 (le formulaire d'inscription), sans quoi la contrainte coûterait une migration.** *Vérifié dans le code :* `TypeProfil` ne connaît que **`fournisseur`, `client`, `les_deux`** — **et « client » reste commercial : quelqu'un qui achète.** *Un besoin est une paire sphère + usage; **un chercheur d'emploi n'a pas de sphère à servir, il a un métier.*** **Trois publics que le corpus prévoit et que le profil n'accueille pas** : une MRC, une chambre de commerce, un chercheur d'emploi. ⚠️ **Et un cas d'usage que les cinq facettes n'ont jamais prévu : RETENIR plutôt que vendre** — *une MRC veut savoir qu'une entreprise de son territoire grandit ailleurs.* **Le grade lui-même change de sens** : « Sur mesure » ne veut pas dire la même chose quand on cherche à EMPÊCHER quelque chose. ✅ **Le mode de défaillance, lui, est corrigé** *(demande de fusion n° 39, 14 septembre)* : `generer_notifications` faisait `if not profile.besoins_fournisseur(): continue` — **un profil sans besoin de type « offre » ne produisait rien, en silence.** Il se signale désormais bruyamment à la création et à l'inventaire, et par une ligne d'exploitation `PROFIL_INCOMPLET` au cycle. *`ProfileNeed.type_besoin` admet déjà autre chose qu'« offre » : **l'axe existe, le moteur ne l'implémente pas.*** |
| D45 | Le produit ne sait dire aucun NIVEAU | Simulations du 14 septembre 2026, rattachée à D46 | ⬜ **avec le portrait de tendance.** *La question : le produit peut-il dire qu'une entreprise est en élan, stable, ou moribonde avec un sursaut récent?* **C'est ce qu'Alexandre voudrait, et le produit n'a pas de quoi le faire.** **Ce qui manque, nommé : un indicateur de NIVEAU.** *Les signaux disent des ÉVÉNEMENTS, jamais un niveau.* **Deux exceptions seulement** : le montant des contrats, et le champ capacité du RACJ. ✏️ **Et le vide s'est réduit le 14 septembre** : le REQ porte une bande d'effectifs *(journal, cas 38)*, ce qui donne au moins un niveau de **TAILLE**, à défaut d'un niveau d'ACTIVITÉ. |
| D46 | Le portrait de TENDANCE — un troisième mode | Décision d'Alexandre, 14 septembre 2026 | ✅ **TRANCHÉE le 14 septembre 2026, à construire.** Consulter un prospect déjà connu et **engendrer un portrait sur les douze derniers mois.** **Un objet distinct, pas une variante** — *le portrait d'opportunité décrit une situation au présent, le portrait de tendance décrit un mouvement sur un an.* **Aucun grade** : le grade mesure la distance entre les faits et ce que l'utilisateur peut servir MAINTENANT; A/AA/AAA ne s'applique qu'au présent. **Même structure, même contrainte de forme, et le second bloc reste** — *« pour un [profil], cette évolution peut représenter… »*. **Douze mois, une seule fenêtre, non paramétrable. Paliers Radar et Radar+** — *c'est une fonctionnalité, pas une source, donc le seuil ne reproduit pas la faille du portefeuille.* ⚠️ **La limite « depuis qu'on regarde » est une contrainte DE CE MODE, pas un détail** : *un portrait sur douze mois d'un produit qui observe depuis six est TRONQUÉ, et il doit le dire.* **« Inchangée depuis qu'on regarde » n'est pas « inchangée depuis 2011 ».** |
| D47 | Le même dossier revient — seconde notification ou mise à jour? | Durée de pertinence par couple, 14 septembre 2026 | ⬜ **avec le chantier 23 (la livraison).** *Conséquence directe de la durée de pertinence PAR COUPLE : si un signal est frais pour une sphère et périmé pour une autre au même instant, **le dossier est déjà destiné à revenir**.* **À trancher : seconde notification, ou mise à jour du dossier existant?** **Une exigence lève le revers quelle que soit l'issue : une seconde notification doit DIRE qu'elle est la suite** — et être consultable avec ce qu'elle prolonge. *Constat de simulation : le portrait de suite est **plus court** que le premier, et c'est normal — il ne redit pas ce qui est connu. Mais il n'est lisible qu'AVEC le premier, et un utilisateur qui aurait supprimé la notification précédente recevrait un texte incomplet. **Ce n'est pas un défaut du portrait : c'est une exigence sur la LIVRAISON.*** |
...
| D51 | Ce que le moteur fait d'un nom PLUS EN VIGUEUR | `docs/CONCEPTION-TRAITEMENT-ARCHIVE-REQ.md`; commit `a362d35` du 17 septembre 2026 | ✅ **TRANCHÉE le 23 septembre 2026 — voir D52**, qui porte la forme retenue et les deux écartées. *L'entrée reste pour son histoire : elle dit pourquoi la question a mis cinq jours à exister.* ⬜ **Déclencheur : après la mesure `outils/noms_perimes_en_production.py`, et AVANT toute nouvelle écriture de NEQ — l'élargissement du statut compris.** **Le 17 septembre, DEUX décisions se tenaient l'une derrière l'autre, et une seule a été prise.** *(1) « Tout entre dans `req_noms` »* — prise ce jour-là : le chargeur a cessé de filtrer `STAT_NOM='V'`, et `req_noms` est passée de 1 505 879 à 5 071 984 formes. ⚠️ *(2) « Ce que le MOTEUR fait d'un nom retiré »* — **jamais prise.** Le commit annonçait que « la règle se pose dans le moteur, pas dans le chargeur »; **elle n'y est pas arrivée** : ni `candidats_par_nom`, ni `_scorer`, ni `resolve_neq_by_name` ne lisent `REQNom.statut`. *Un nom retiré est donc aujourd'hui une porte à PLEINE FORCE, indiscernable d'un nom en vigueur, et `tests/test_reresolution_neq.py` verrouille ce comportement.* **TROIS FORMES, aucune retenue :** *(a)* **plafonner le score** d'un appariement obtenu par un nom retiré; *(b)* **ne le consulter qu'en SECOND TEMPS**, après échec du premier; *(c)* **le laisser tel quel** — un nom abandonné mène souvent à la bonne entreprise, et le coût serait de perdre ce que le 17 septembre a gagné. ⚠️ **Ce qui distingue le risque : un nom retiré porté AUJOURD'HUI, en vigueur, par un AUTRE NEQ.** *Une entreprise retrouvée sous son propre ancien nom est juste; c'est la reprise du nom qui déplace une identité.* **La mesure sépare les deux cas et ne tranche pas entre eux.** ⚠️ **Pourquoi cette entrée arrive cinq jours en retard** : la phrase qui la qualifiait de décision — *« c'est une règle, donc une décision d'Alexandre »* — vivait **hors du corpus**, donc la section 15 de la charte ne s'y est jamais appliquée. *C'est la faille I, sur le produit lui-même.* |
| D52 | Le TROISIÈME TEMPS — quand le moteur consulte un nom retiré | D51; mesure `outils/noms_perimes_en_production.py` du 22 septembre 2026 | ✅ **TRANCHÉE le 23 septembre 2026 — le second temps.** *« `resolve_neq_by_name` ne consulte les noms retirés que si aucun nom en vigueur n'a trouvé l'entreprise. Je ne plafonne pas les scores : ça ferait perdre des changements de nom qui sont justes. »* **L'EXPOSITION QUI L'A DÉCIDÉE : 208 dossiers sur 8 742 (2,4 %) trouvés par un nom retiré — dont 157 par leur PROPRE ancien nom et 51 par un nom qu'une autre entreprise porte aujourd'hui.** **DEUX FORMES ÉCARTÉES, et leur motif :** ⛔ *plafonner le score d'un appariement obtenu par un nom retiré* — **elle punirait les 157 pour atteindre les 51**, et un changement de nom juste est le cas le plus fréquent, pas l'exception; ⛔ *laisser tel quel* — **le risque n'est pas nul et il est nommé** : un nom abandonné par un NEQ et repris par un autre déplace une identité, et rien ne le signalerait. ⚠️ **CE QUE LA FORME RETENUE GARANTIT SANS MESURE** : écarter des formes ne peut qu'ABAISSER des scores, donc le premier temps ne retient jamais moins que le troisième, et **quand il ne retient rien, le troisième rejoue exactement le comportement d'avant**. *La règle ne change l'issue que là où un nom EN VIGUEUR désigne quelqu'un d'AUTRE* — **« perdre son NEQ » est impossible par construction, et `outils/second_temps_des_noms_retires.py` le vérifie sur la population plutôt que de s'en remettre au raisonnement.** ⚠️ **Elle vit dans la RÉSOLUTION, donc elle s'applique à chaque import, sans relecture** — c'est ce qu'on lui demande. *Et elle ne réécrit rien toute seule : un NEQ posé reste posé tant qu'une passe de reprise ne le rouvre pas (étape 14 du bloc de l'import).* **Un code de statut INCONNU compte comme RETIRÉ** *(`nom_retire`)* : le coût de l'erreur est borné, puisque la forme revient au troisième temps. |
| D53 | L'ÉLARGISSEMENT du statut aux confirmations, et sa condition | Chantier 3+4 §9bis; mesure du 21 septembre 2026 | ⬜ **Le déclencheur A FIRÉ** — les quatre constats du 21 septembre ont reçu leur réponse, donc la décision est DUE. **494 dossiers**, à la condition stricte que le statut désigne le MÊME candidat que le score. ⚠️ *La réserve ne se lève pas : le statut et le score comparent les candidats entre eux, jamais le gagnant au dossier.* **Et le chiffre qui devait la trancher a changé de sens** : les « 81 contradictions de l'adresse » sont DEUX faits, et une part d'entre elles est prononcée par la VILLE, qui compare des graphies. *Ce qui reste à lire avant de trancher : la ventilation des « exclut tous » par niveau.* |
| D54 | Le critère qui fait passer une règle d'OUTIL PONCTUEL à RÈGLE PERMANENTE | Chantier 3+4 §17bis, étape 10 | ⬜ **Déclencheur : avant l'intégration de la PREMIÈRE règle à la résolution** *(étape 10 du bloc de l'import)*. **Jamais discuté.** *Une échelle — par exemple la part de contradictions sous laquelle une règle est acceptée.* ⚠️ **Probablement la même décision que D55**, et les deux se posent ensemble. |
| D55 | Le TAUX D'ERREUR ACCEPTABLE pour qu'une règle tourne seule | Chantier 3+4 §13, point 10 — la mesure de justesse | ⬜ **Déclencheur : avec D54, avant l'étape 10.** *On a compté combien de NEQ on posait, jamais combien sont bons.* ⚠️ **Un NEQ posé sur la mauvaise entreprise est PIRE qu'un dossier sans NEQ** : il produit un faux prospect, et rien ne le signale. *La mesure elle-même est un livrable du chantier 4; l'échelle qu'elle demande est ici.* |
| D56 | La DÉRIVE PAR RADIATION — une radiation entre deux zip fait écrire un NEQ sans décision | Chantier 3+4 §17bis, étape 10 | ⬜ **Déclencheur : avant l'étape 10.** *Quand la règle du statut tournera seule, un dossier passera de « plusieurs candidats » à « écriture » **parce qu'un concurrent a été radié entre deux imports**.* ⚠️ **`REQEntry` ne porte AUCUNE date de radiation** : rien ne distinguera une radiation d'hier d'une radiation de dix ans. **La règle convertirait « radiation observée » en « NEQ posé », en silence.** |
| D57 | La COUPE À CINQ CANDIDATS — son échelle | Chantier 3+4 §12bis; `resolve_neq_by_name(limit=5)` | ⬜ **Déclencheur : avant la correction** *(étape 13 du bloc de l'import)*. **Le lot soumis au score est plafonné à cinq, après une récupération qui en ramène jusqu'à 2 000.** ⚠️ *Une valeur, donc une échelle, donc Alexandre.* **Vu 2 fois sur 32 paires lues : le bon candidat était sous la coupe.** *Et elle a déjà fait entrer un faux fait dans une mesure — un axe « aucun dossier n'a plus de 5 candidats » qui était le plafond de l'instrument.* |
| D58 | Le PLANCHER DE REMPLISSAGE par fichier, à l'import du REQ | Chantier 3+4 §17bis, étape 3 | ⬜ **Déclencheur : avant l'import.** *Une part retenue qui s'effondre doit refuser bruyamment.* ⚠️ **C'est une SECONDE échelle, distincte des seuils de quarantaine de D17.** ⚠️ *Et sa valeur de référence a déjà bougé* : le « ~32,7 % de lignes en vigueur » de la conception est une propriété du FICHIER — **le chargeur les garde toutes depuis le 17 septembre, donc la part retenue est ~100 %.** *Poser un plancher sur l'ancienne valeur refuserait chaque import.* |
| D59 | `RawSignal` n'a aucun emplacement pour un CODE POSTAL | Chantier 3+4 §17 | ⬜ **Déclencheur : avant tout code qui s'y appuie.** *`RawSignal` porte `adresse`, `ville`, `region`.* ⚠️ **Le niveau le plus fort du départageur — 1 764 départages, dont 1 735 écrits — ne fonctionne que parce que `faits_du_dossier` va chercher le code postal lui-même dans `Signal.champs`.** **Deux directions : élargir `CLES_ADRESSE`, ou donner un emplacement à `RawSignal`.** *Décision de MODÈLE, aucun dossier en jeu.* ⚠️ *Et un second usage l'attend : le rayon d'action de l'utilisateur.* |

| D60 | La LONGUEUR MINIMALE d'une forme au scorage — son échelle, et ce qu'on appelle « longueur » | Chantier 3+4 §12bis; `outils/longueur_des_formes.py` | ⬜ **Déclencheur : avant toute nouvelle écriture de NEQ.** *Rien ne borne aujourd'hui la longueur d'une forme du registre au scorage.* ⚠️ **`WRatio` rend au moins 90,0 quand un nom du registre est entièrement contenu dans le nom détecté, et DEUX LETTRES suffisent** — `« LA »` a scoré **95,0** sur `#5476`. *Une valeur, donc une échelle, donc Alexandre* — **qui la posera avec Claude, et ne veut aucune valeur posée d'avance pour débloquer.** ✅ **L'UNITÉ EST TRANCHÉE le 23 septembre 2026 — LA LETTRE.** *« C'est la seule qui traite `« LA »` et `« L.A. »` comme la même porte. »* **`« L.A. »` se normalise en `« l a »` — 3 caractères, 2 lettres, 2 mots**, et une borne en CARACTÈRES retirerait l'une en gardant l'autre. ⬜ **La VALEUR attend la mesure**, qu'Alexandre lance ensuite. **`outils/longueur_des_formes.py` rend la distribution, le coût de chaque borne et les trois mesures; il ne propose rien.** |
| D61 | Au-delà de deux prétendants, le DÉTENTEUR garde-t-il son NEQ ? | Chantier 3+4 §12bis; `falkye/resolution.py::garde_des_pretendants` | ✅ **TRANCHÉE le 23 septembre 2026 — le détenteur garde son NEQ.** *« Le produit conserve, il n'efface pas. Le nouveau venu reste un dossier séparé, sans NEQ, et le refus est journalisé. »* **La garde est descendue dans la résolution le 23 septembre, et sa FORME a changé en descendant** : la passe par lot refuse à TOUS, la résolution refuse au NOUVEAU VENU. ⚠️ **`Company.neq` est UNIQUE** — le partage n'est pas une issue, et retirer le NEQ au détenteur serait **une écriture qui EFFACE une identité**, déclenchée par l'arrivée d'un signal. ⛔ **FORME ÉCARTÉE : révoquer au-delà de deux.** *Elle ferait dépendre la perte d'une identité déjà posée de l'ordre d'arrivée des signaux — et le produit conserve.* **Le rang reste ÉCRIT au journal**, pour que ces NEQ soient visibles. ⚠️ *Conséquence tenue* : **les 26 dossiers visant `8879690699` restent SANS NEQ** jusqu'à D27 et D28. |
| D62 | La TROISIÈME FORME du troisième temps — rejouer le pipeline depuis le préfixe | D52; mesure `outils/second_temps_des_noms_retires.py` §5 du 23 septembre 2026 | ✅ **TRANCHÉE le 23 septembre 2026 — NON. On garde la forme actuelle.** *La forme actuelle rejoue les formes d'avant sur le lot DÉJÀ ÉLARGI; la troisième forme repartirait du PRÉFIXE quand le troisième temps tire.* **LA MESURE QUI L'A DÉCIDÉE : 13 dossiers récupérés contre 29 perdus**, sur les 6 468 où le troisième temps tire. ⚠️ **Et le motif n'est pas seulement le solde** : *ce que la troisième forme récupère, elle le retient **en ignorant un concurrent** que le lot élargi avait vu* — **exactement la façon dont l'ancienne règle retenait les 9 pertes.** **Un gain obtenu par ignorance n'est pas un gain.** ⚠️ *Inscrite pour que la question ne se rouvre pas dans un mois : un nouveau chiffre ne suffira pas, il faudra une mesure de JUSTESSE (§13, point 10).* |
| D63 | Les 35 CHANGEMENTS de NEQ que le troisième temps produirait | Chantier 3+4 §5bis; `outils/second_temps_des_noms_retires.py` §1 | ⏸️ **EN SUSPENS — décision du 23 septembre 2026, non appliquée.** *« Poser une identité et en REMPLACER une ne sont pas le même geste. »* ⚠️ **Et plusieurs iraient vers une entreprise RADIÉE** — `#594`, `#1728`, `#4949`. **Déclencheur : l'étape de RÉOUVERTURE, avec la règle du STATUT par-dessus.** ✅ *La garde est structurelle, pas une consigne* : `outils/reresolution_neq.py` ne balaie que `Company.neq IS NULL` — **une reprise ne peut pas remplacer un NEQ**, et un test le verrouille. |
| D64 | Les formes NON QUALIFIÉES entrent au PREMIER temps — quelle garde, et sur quel axe | Chantier 3+4 §5quinquies, §8; `falkye/sources/req.py::nom_retire` | ⬜ **Déclencheur : avant la prochaine pose de NEQ, mesure en main.** *`nom_retire('?')` rend `False`* — **les 125 767 formes sans colonne de statut** (`DENOMN_SOC` 93 816, `NOM_ETAB` 31 951 au 22 septembre) **entrent au premier temps, à égalité avec un nom en vigueur.** ⚠️ *Un gisement sans statut n'est pas RETIRÉ, il n'est pas QUALIFIÉ, et les deux ne se confondent pas.* ⛔ **L'axe du GISEMENT est écarté, et par une mesure déjà faite** : §8, 21 septembre — *« le gisement de la forme gagnante comme garde-fou ne sépare pas les bons des faux »*, close. **Et `NOM_ETAB` rend 5 récupérations sur 2 517, toutes des entreprises individuelles — pour lesquelles le nom de la personne EST la raison sociale.** *L'axe proposé à la place est la GRAPHIE (`NOM, PRÉNOM`), comptée par `reresolution_neq` sans rien gouverner.* **Le fait qui l'ouvre : `#3705 Ferme Martin Bouchard` rattaché à « Éditions Melançon » par « BOUCHARD, MARTIN ».** |
| D65 | `#782 Science & Humans` → `GLOBAL ZEBRA HOLDINGS INC.`, par le nom ANGLAIS | Chantier 3+4 §5quinquies | ⏸️ **LAISSÉ PASSER le 24 septembre 2026, en le nommant.** *Entré par `NOM_ETRNG`, ville **Toronto**; **le seul de ce type dans les 40 paires lues**.* ⚠️ **Déclencheur de réouverture : un SECOND cas du même type.** *Nommé plutôt que toléré en silence — une décision non tranchée disparaît (charte, section 15), et un cas toléré sans trace se retrouve défendu par l'usage.* |
**64 entrées, 17 tranchées.** *Relu le 24 septembre 2026 en ajoutant D64 et D65 : 62 → 64 entrées, aucune neuve tranchée — D64 est ⬜ (elle attend sa mesure) et D65 est ⏸️ (elle attend un second cas).* ⚠️ **Deux entrées ne comptent PAS comme tranchées, et il faut le dire** : **D63** est ⏸️ *en suspens* — la question se REPOSERA à la réouverture, elle n'est pas close; **D60** garde son ⬜ — *son UNITÉ est tranchée (la lettre), sa VALEUR ne l'est pas*, et une entrée à moitié tranchée se compte comme ouverte. *Compte relu dans le tableau le 22 septembre 2026 — il annonçait
...
objet. *Les décisions marquées 💤 ne se datent pas tant que cette portée tient.* **Appliquée dans le code le 11 septembre 2026** : la passe de détection d'expansion inter-provinciale ne s'exécute plus tant que le registre actif porte moins de deux provinces — *garde DÉRIVÉE du registre, jamais un drapeau posé à côté, donc elle se lève d'elle-même le jour où une source d'une autre province repasse `actif`.* **Non-geste mesuré avant d'être posé** : aucune entreprise de la base ne vient des trois sources provinciales en veilleuse — *0 sur 11 556*. ⚠️ **La garde porte sur la PASSE, pas sur la lecture d'un lien déjà en base** : posée aux deux endroits, elle a été resserrée le jour même, trois tests ayant montré qu'un lien acquis cessait de donner son bonus. *Ne plus créer et cesser de lire sont deux gestes différents.* *Le compte se relit dans le tableau, il ne se recopie nulle part.* Une
...
## Ce que neuf simulations de portrait ont ajouté à cet ordre — 13 et 14 septembre 2026

**L'appariement n'est plus un chantier parmi d'autres, et c'est le résultat le plus net.** *Sans NEQ :
**pas de secteur**, donc pas de raison d'être dans le portrait; **pas de dossier stable**, donc pas de
mémoire; **pas de croisement**, donc pas d'association.* **Les trois facettes du cerveau dépendent d'un
même verrou.**

**Aucune source nouvelle n'a manqué.** Les portraits ont tenu avec l'existant plus l'OQLF, le RACJ, le
CIPO et les permis de Montréal — **tous déjà au catalogue**. *Ce qui manquait : l'appariement, le secteur
promu au dossier, et des lectures qui n'existent pas.*

- **Le secteur du REQ a porté une association dans CHAQUE simulation** — *la seule qui ne vienne d'aucun
  signal : elle vient de ce que l'entreprise EST.*
- **Les établissements du REQ ont porté trois lectures**, et le relevé dit qu'ils n'atteignent jamais le
  dossier.
- **L'OQLF a porté deux simulations**, pour deux raisons distinctes — le NEQ sans appariement, et le seuil
  de 25+ daté. *Cinquième dans l'ordre du catalogue à cause de D5; les simulations suggèrent qu'elle
  devrait être première. À trancher, pas à faire.*
- ✏️ **Et la neuvième a ouvert une lecture que les huit premières ne pouvaient pas produire** : *un contrat
  de 1,8 M$ pour une entreprise déclarée à **26-49 employés**.* **La DISPROPORTION est calculable pour la
  première fois**, parce que la bande d'effectifs du REQ existe *(journal, cas 38)*.

### Le livrable 1 du chantier 12 remonte — le préalable, pas le chantier

**Avis rendu le 14 septembre 2026, décision d'Alexandre.**

**Pour, tout de suite :** *(1)* l'audit dit déjà que le 12 **« peut se mener en parallèle du socle — le
seul chantier qui peut avancer pendant que le reste se répare »**; *(2)* son préalable était **à moitié
fait**; *(3)* il **sert déjà un autre point du plan** — la correspondance code → sphère *(D48)*, qui
attend la même matière; *(4)* il **ne dépend pas de l'appariement**, donc il n'est pas derrière le verrou.

⚠️ **Contre, pour le RESTE du 12, et c'est le corpus qui le dit.** Le livrable 5 — *la règle de
réfutation, « la partie non négociable »* — exige que **chaque lien se teste contre l'historique réel**,
et l'audit précise que c'est le **chantier 13** qui rend ce garde-fou *« opérationnel plutôt que
théorique »*, par le taux de rejet. **Or le taux de rejet demande des utilisateurs qui rejettent**, et les
cycles récents produisent zéro notification. *Sans lui, le 12 propose des liens et n'a rien pour les
réfuter — exactement ce que sa propre règle interdit.*

**Donc : remonter le livrable 1, laisser les livrables 2, 3 et 5 où ils sont.** *Ce qui remonte est ce qui
ne dépend de rien; ce qui attend est ce qui a besoin d'usage réel.* **Et la réserve du mandat vaut d'être
reprise au moment de trancher : le 12 ne créera pas de signal là où il n'y en a pas — il révèle des
signaux captés mais mal attribués.**
...
## Relevé du second rôle des sources — pour D41 et D42

*Écrit le 11 septembre 2026, en répondant à une question qu'aucun document ne posait sous cette forme.*

**Une source a DEUX rôles, et le corpus n'écrivait que le premier.** *Le signal qu'elle produit* — cela,
le registre et les fiches le disent. Et *ce qu'elle ajoute au dossier cumulatif une fois le signal
détecté*. **Une source peut remplir l'un sans l'autre**, et deux l'ont fait en sens contraire *(D42)*.

**Ce que le relevé mesure, exactement.** Les champs qu'un connecteur met dans `RawSignal` et que
`resolve_company` recopie sur l'entreprise. **Le reste du `champs` est conservé et n'entre jamais dans le
dossier** — c'est une distinction de mécanisme, pas de vocabulaire *(voir l'EIMT ci-dessous)*.

| source ingérée | promu au dossier | ce qui s'en sert |
|---|---|---|
| `permis_construction_laval` | **adresse, ville, région** | géocodage, formateur, premier contact, résolution NEQ |
| `rob_top_growing` *(en veilleuse, D42)* | **ville, région, secteur** | + synthèse et classement client-cible, par le secteur |
| `deloitte_fast50` *(en veilleuse, D42)* | **ville, région** | résolution NEQ, dédoublonnage, filtre territorial |
| `subventions_federales` | **ville, région** | idem |
| `eimt` | région seule | filtre territorial, correspondance par territoire |
| `seao` | **rien** | — |
| `investissement_quebec` | **rien** | — |
| `contrats_federaux` | **rien** | — |

**Trois des huit n'ajoutent rien au portrait** : elles produisent un signal et repartent. *Ce n'est pas un
défaut en soi — un signal est une raison suffisante d'exister — mais personne ne l'avait écrit, et on ne
peut pas arbitrer une source sur un rôle qu'on ne sait pas qu'elle ne remplit pas.*

**Et aucune des huit ne renseigne jamais `site_web`.** Seul l'enrichissement web le fait — celui qui
échoue en 403 et occupe seize minutes sur vingt-neuf en régime.

**Le champ qui pèse le plus est la VILLE**, et c'est ce qui a rendu D42 difficile : +5 au score de
résolution NEQ, +5 au dédoublonnage, et **le départage quand deux entrées du REQ se disputent un nom** —
donc elle agit sur le taux d'appariement, *le mur qui reste après D15*.

**⚠️ Un défaut constaté en faisant ce relevé, et il n'appelle aucune décision — seulement une
correction.** `falkye/sources/eimt.py` capture l'adresse de l'employeur **dans `champs`** et ne la promeut
pas en `RawSignal.adresse`. `resolve_company` ne recopie que les champs nommés : **`company.adresse` reste
donc vide**, et le géocodage, le formateur et le premier contact ne la voient jamais. *De l'enrichissement
capturé, conservé, et inutilisé — sur la source qui touche 75 % des entreprises de la base.*

### Complément du 13 septembre 2026 — le SEAO, le REQ, et la profondeur qui manquait

**SEAO — promu au dossier : RIEN.** *Jamais LUS alors qu'ils sont dans l'avis* : **la classification
normalisée** (aucune lecture de `items[].classification`) et **l'identifiant du donneur d'ouvrage** (seul
son NOM était pris). *Conservés sans usage* : la description des besoins, la devise, le statut
d'attribution. *Utilisés sans être promus* : le nom du donneur (message de premier contact), le titre de
l'avis (mots-clés du profil; motif de la notification), le montant (`valeur_associee`, paliers absolus du
score).

⚠️ **C'est la cause directe de l'uniformité des signaux.** *Le mandat du chantier 22 nomme la
classification normalisée comme l'antidote au fait que tous les signaux portent la même sphère — le SEAO
la porte, et on ne s'en servait pas.* **La sphère vient du `signal_type_id`, donc elle est la même pour
tout avis, quel que soit l'objet du contrat.** *(Capté depuis le 14 septembre; interprété, non — D48.)*

**REQ — promus au dossier** *(par `_enrich_from_req`, seulement si le champ est vide **et** si l'entreprise
est RÉSOLUE)* : nom officiel, statut légal, adresse, ville, code postal, **secteur d'activité — code ET
libellé**. **Donc le produit PORTE le secteur.** *La réponse à « faut-il une source pour ça » est non : il
faut l'appariement.* **Le secteur manque exactement pour les entreprises non résolues — le mur de D15, et
pas une autre raison.**

⚠️ **La région, elle, n'arrive JAMAIS par le REQ** : `REQEntry.region` est écrit à `None`, faute de région
administrative dans le vrai schéma. *Seuls les connecteurs en donnent une — et c'est sur `raw.region` que
porte le filtre territorial (D41).*

**REQ — les établissements n'atteignent pas le dossier.** Ils sont lus *(adresse, ville, code postal,
secteur, nom, principal)* et servent un **grain de diff distinct** qui émet
`nouvel_etablissement_secondaire`. **Mais *combien, où, depuis quand* ne se lisent nulle part par
entreprise.** *Le catalogue dit que plusieurs nouveaux établissements croisés avec un autre signal
deviennent très forts — ce croisement n'a aujourd'hui aucune donnée à croiser, sinon les signaux
eux-mêmes.* **Et « depuis quand » est borné par le premier import.**

⚠️ **Ce relevé n'est PAS assez profond pour le chantier 12, et la distinction est structurelle.** *Il
inventorie ce que chaque source **PROMEUT AU DOSSIER** — le second rôle. Le 12 a besoin de ce que chaque
source **CAPTE**, `champs` compris.* **Sur le SEAO, les deux ensembles ne se recoupent presque pas** :
promu au dossier = **rien**; capté = **quinze clés**. *Et c'est précisément là que vit la démonstration de
l'audit — l'assurance est déclarée sans source alors que **six champs déjà captés portent son
déclencheur**, et ces six-là sont dans `champs`, invisibles à ce relevé.* **Il faut refaire les sources en
profondeur, pas compléter celles-ci en surface** — voir le chantier 12, livrable 1, livré le
14 septembre.
...
- **Fait réel : aucun.** *Corrigé le 11 septembre 2026.* Le relevé attribuait ce grade à un passage de
  `A/B/C` à `A/AA/AAA` et l'appelait **la décision la plus instructive du corpus sur les échelles**.
  **Ce passage n'a jamais eu lieu** — c'était un exemple donné en conversation, consigné au journal
  comme un fait *(journal, cas 16)*. **Le grade de pertinence — la seule finalité présentée du produit —
  n'a donc aucun fait fondateur**, et c'est la correction qui a le plus changé la lecture de ce relevé.
...
### Livrable 1 — ✅ LIVRÉ le 14 septembre 2026, et sa forme fait partie du livrable

*L'inventaire de champs par source active, approuvé mais jamais livré, est fait. Trois exigences sont
apparues en le construisant, et chacune a été payée par une erreur.*

**Il est GÉNÉRÉ depuis le code, jamais écrit à la main.** *Un connecteur a changé le jour même; une liste
de champs recopiée serait fausse au premier commit suivant.* **C'est la règle du corpus contre les copies,
appliquée à un inventaire.**

**`outils/inventaire_champs.py`** — les sources actives sont lues **au registre**, les champs déclarés
**dans l'arbre syntaxique** du connecteur, le remplissage **en base**. *Sept connecteurs, une quarantaine
de champs déclarés.*

⚠️ **Deux colonnes, et la seconde est celle que la journée a payée pour apprendre** : *(a)* ce que le code
**DÉCLARE** capter, *(b)* le **taux de remplissage RÉEL**. **Un inventaire tiré du code seul aurait listé
`description_tender` comme un champ disponible pour la règle d'assurance — et il était vide dans 100 % des
cas.** *Un champ déclaré n'est pas un champ rempli, et le 12 construirait ses liens sur du vide sans s'en
apercevoir.* **Ça rend aussi le livrable 4 possible** *(normaliser le texte libre déjà capté)* : la
normalisation a besoin de savoir quels champs sont du texte libre et combien de valeurs distinctes ils
portent — deux choses que seule la mesure donne.

**Pourquoi l'arbre syntaxique et non une expression régulière, et ce n'est pas théorique** : une
expression sur `"clé":` attrapait les dictionnaires **voisins** — `colonnes`, `exemple`,
`taille_decompressee_octets` de l'inspection de zip du REQ — **et le 12 aurait cru disposer d'une matière
qui n'existe pas.** *Elle manquait aussi un champ réel d'`investissement_quebec`.* **Un test le
verrouille.**

**Trois écarts nommés par la sortie, et chacun veut dire autre chose** : *déclaré jamais rempli* (le
connecteur écrit une clé que la source ne porte pas) · *rempli non déclaré* (une expansion `**` ou un
chemin oublié) · *déclaré non ingéré* (**absence de mesure, pas champ mort**).

⚠️ **Le REQ porte des clés DYNAMIQUES** *(`**etab`, `**chgt`)* : **sa liste déclarée est incomplète par
nature**, et l'outil le signale plutôt que de rendre une liste qu'on croirait complète. *C'est la source la
plus riche, et celle dont le code dit le moins.*

**Et l'outil n'attribue AUCUNE sphère.** *Le diagnostic à rebours est le livrable 2; il exige la
réfutation du livrable 5, opérationnelle seulement avec le 13. Proposer un lien ici produirait exactement
ce que la règle de réfutation existe pour empêcher.*

### La troisième colonne — ce que la source PORTE et que personne ne demande

*Ajoutée le 14 septembre 2026. `outils/sonde_source.py`.*

**Les deux premières colonnes comparent ce que le code déclare à ce qu'il obtient. La troisième compare
ces deux-là à ce que la SOURCE offre.** *C'est le seul endroit d'où peut venir une matière que personne
n'a demandée.*

**Ce qu'elle a trouvé, en une commande par source :**

| source | champs remplis | champs lus | trouvaille |
|---|---:|---:|---|
| `contrats_federaux` | 43 | 10 | **`economic_object_code` — 100 %, 47 valeurs** : l'équivalent fédéral de l'UNSPSC *(D48)*. Plus `number_of_bids` (mesure de concurrence), `vendor_postal_code`, `delivery_date`, `solicitation_procedure`. |
| `subventions_federales` | 35 | 15 | **`recipient_type`** — *une typologie de bénéficiaire, matière directe pour **D28***. **`recipient_operating_name`** — *le nom d'USAGE à côté du nom légal, **un second nom à apparier contre le REQ**, donc le mur de D15*. **`recipient_business_number`** — *un identifiant fédéral, matière pour **D29***. `recipient_postal_code` à ~89 % *(l'adresse est une exigence de sortie)*, `prog_purpose_fr` et `expected_results_fr` à ~90 %. |

**Une honnêteté à garder** : `naics_identifier` — le code sectoriel normalisé, celui qu'on espérerait —
**n'est rempli qu'à 10 %**. *Rien à voir avec l'UNSPSC du SEAO à 93,6 %.* **La troisième colonne sert
justement à ne pas confondre « le champ existe » et « le champ est rempli ».**

**Ce qu'elle coûte, et ce qu'elle ne couvrira jamais** : elle demande **une stratégie d'accès par source**
— API datastore, CSV en vrac, archive zip, page grattée. *Les quatre existent déjà dans le code, mais
séparément.* ⚠️ **Les sources grattées ne rendront jamais une liste de champs**, seulement ce que le
connecteur a su extraire : **pour celles-là, la troisième colonne est vide PAR NATURE, et la sortie le
dit.**

⚠️ **Un défaut de la sonde, trouvé en la comparant à une mesure de la veille, et corrigé** : sans tri, le
datastore rend les enregistrements **les plus anciens** — *29 champs remplis contre 35 sur les récents.*
**Une sonde qui échantillonne le passé sous-estime ce que la source porte aujourd'hui, et rien dans sa
sortie ne le disait.** Elle trie désormais sur `_id desc` **et l'écrit dans sa provenance**. *Un chiffre
donné au corpus en est sorti faux et a été corrigé avant écriture : `amendment_date` est remplie à
**27,8 %**, pas 0,2 %.*
...
### Ce que neuf simulations ajoutent au 23 — 14 septembre 2026

**Le dossier est DÉJÀ destiné à revenir, et c'est une conséquence de la durée de pertinence par couple.**
*Si un signal est frais pour une sphère et périmé pour une autre au même instant, le même dossier se
represente.* **À trancher : seconde notification, ou mise à jour du dossier existant?** *(registre, D47.)*

**Une suite doit être consultable avec ce qu'elle prolonge.** *Constat de simulation : le portrait de
suite est **plus court** que le premier, et c'est normal — il ne redit pas ce qui est connu.* **Mais il
n'est lisible qu'AVEC le premier**, et un utilisateur qui aurait supprimé la notification précédente
recevrait un texte incomplet. **Ce n'est pas un défaut du portrait : c'est une exigence sur la
LIVRAISON** — la suite doit dire qu'elle est une suite, **et rendre le précédent atteignable**.
...
### La contrainte du formulaire — à écrire AVANT que l'interface existe

*Ajoutée le 14 septembre 2026, sur précision d'Alexandre.*

**Un profil sans besoin déclaré ne doit jamais pouvoir être enregistré par un utilisateur.** *Le correctif
du 14 septembre (demande de fusion n° 39) couvre le chemin **administratif** — `profile create` puis
`add-need` : là, le profil vide est nécessaire, et le silence devait y être nommé de toute façon.* **Mais
le jour où un utilisateur s'inscrit LUI-MÊME, il remplit un formulaire, pas deux commandes** — et là, les
**champs obligatoires empêchent l'enregistrement** : pas de sauvegarde sans au moins un besoin déclaré.
**Le profil incomplet n'existe jamais.**

⚠️ **Écrire la contrainte maintenant, pas après** : posée après coup, elle coûterait une migration de
profils déjà entrés.

**Et elle dépend de D44** *(ce qu'un profil peut DÉCLARER)* : *rendre un besoin obligatoire n'a de sens
que si les types de profil couvrent les publics qu'on veut servir — une MRC, une chambre de commerce, un
chercheur d'emploi n'entrent dans aucun des trois types actuels.*
...
- ⬜ Chemin d'import du miroir — deux allers-retours par ligne, mesuré. **Et le pic mémoire de 3 535 Mo
  vient d'ailleurs : un seul `commit()`, à la toute fin** *(`falkye/sources/req.py`)*. **La correction
  identifiée le 11 septembre est une validation par lots** — un `commit()` tous les *N* upserts ferait
  tomber le pic **sans toucher à la durée ni au nombre d'énoncés**. *Écrit ici plutôt que fait : ce
  n'était pas le chemin du jour, et 3 535 Mo sur 8 Go passe. Le tranchage par `limit` a été écarté le même
  jour, pour trois raisons : `import_miroir_req.py` ne l'expose pas, **`Etablissements.csv` est chargé en
  entier quel que soit `limit`**, et **il n'y a pas d'`offset`** — une deuxième tranche repartirait de la
  ligne 1. L'upsert par NEQ la rend idempotente, donc rien ne se corrompt, mais deux tranches coûteraient
  plus qu'un bloc.*
