...
Le moteur (falkye/engine.py) ne connaît QUE cette interface — jamais une source
...
if TYPE_CHECKING:
    ...
    from falkye.registry.loader import SourceDef
...
class SourceConnector(ABC):
    ...
    def detect(self, since: datetime | None, db_session: "Session") -> Iterator[RawSignal]:
        ...
        falkye/sources/req.py). Les autres connecteurs l'ignorent : ils restent
    ...
    def detect_from_file(self, path, db_session: "Session", *, limit: int | None = None) -> Iterator[RawSignal]:
        ...
        — puis l'importe). Complète falkye/manual_import.py, qui couvre le cas
